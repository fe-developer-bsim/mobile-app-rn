...
import {
  sendTrackingFivePointTwo,
  sendTrackingFivePointFive,
  sendTrackingFivePointFour,
  getInstalledApp,
} from './loan.thunks';
...
export const setFormValue = (fieldName, value) => dispatch =>
  dispatch(change('EForm', fieldName, value));

export const getDistrictByCity = cityName => dispatch =>
  dispatch(getAreaData('district', cityName, 'getDistrictList'));

export const getCityByProvince = provinceName => dispatch =>
  dispatch(getAreaData('city', provinceName, 'getCityList'));

export const getDistrictBySubDistrict = subDistrictName => dispatch =>
  dispatch(getAreaData('subDistrict', subDistrictName, 'getSubDistrictList'));

export const getWorkTypeList = jobType => dispatch =>
  dispatch(getAreaData('jobType', jobType, 'getWorkList'));
...
export const getCityList = (provinceData, onClearChange) => dispatch => {
  ...
  return dispatch(getCityByProvince(provinceCode))
    .then(res => {
      dispatch(actionCreators.hideSpinner());
...
export const getDistrictList = (cityData, onClearChange) => dispatch => {
  ...
  return dispatch(getDistrictByCity(cityCode))
    .then(res => {
...
export const getSubDistrictList = (districtData, onClearChange) => dispatch => {
  ...
  return dispatch(getDistrictBySubDistrict(districtCode))
    .then(res => {
...
export const getJobList = jobType => dispatch => {
  ...
  return dispatch(getWorkTypeList(jobCode))
    .then(res => {
...
export const handleSubmit =
  ...
  (dispatch, getState) => {
    ...
    const isEmoneyKyc =
      !isEmpty(find(accountList, {accountType: 'emoneyAccount'})) &&
...
export const generatePhoto =
  (dataUser, fieldName, base64, pageName, formid) => (dispatch, getState) => {
    dispatch(actionCreators.showSpinner());
    const state = getState();
...
export const confirmImage =
  (dataUser, fieldName, pageName, formid, data) => dispatch => {
    const base64 = result(data, 'base64', '');
    const nextFunction = dispatch(
      wrapDispatchMethodInFunction(
        generatePhoto,
        dataUser,
        fieldName,
        base64,
    ...
    dispatch(
      NavigationActions.navigate({
        routeName: 'ImageConfirmation',
        params: {data: base64, nextFunction},
      }),
    );
  };
...
export const confirmImageRetake = data => dispatch => {
  ...
  dispatch(
    NavigationActions.navigate({
      routeName: 'ConfirmRetakeSelfiePage',
      params: {data: base64},
    }),
  );
};
  ...
  return (dispatch, getState) => {
    ...
    api
      ...
      .then(res => {
        ...
        if (responseCode === '00') {
          dispatch(
            actionCreators.saveLoanProduct(result(res, 'data.config', [])),
          );
          dispatch(
            NavigationActions.navigate({routeName: 'ChooseLoanAccount'}),
          );
...
export const getFirstPage =
  (dataDukcapil, dataCore, flagBack = false, pageNameBack = '') =>
  (dispatch, getState) => {
    dispatch(actionCreators.showSpinner());
    ...
    const state = getState();
    const ktpId =
      result(dataDukcapil, 'NIK', '').toString() === ''
    ...
    return api
      ...
      .then(res => {
        ...
        if (result(res, 'data.responseCode', '99') === '00') {
          ...
          dispatch(actionCreators.hideSpinner());
          Toast.show(
            getErrorMessage(res, language.APP_ERROR__TITLE),
            Toast.LONG,
          );
        }
      })
