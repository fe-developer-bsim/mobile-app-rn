...
export function saveLoanDataPgoAcceptOnly() {
  return (dispatch, getState) => {
    ...
    const payload = {
      requestData,
      targetUrl: targetUrlThreePointOne,
      type: 'post',
    };
...
export function sendTrackingFivePointFive(eventCodeRaw = ' ') {
  ...
  return (dispatch, getState) => {
    ...
    const platformType =
      result(Platform, 'OS', 'android') === 'android' ? '2' : '1';
        ...
        if (data === 'aktif') {
          ...
          dispatch(
            NavigationActions.navigate({
              routeName: 'CreateSignWebViewPage',
              params: {url: data},
            }),
          );
        }
  ...
  return (dispatch, getState) => {
    ...
    return api
      ...
      .then(res => {
        ...
        const payloadLast = {
          requestData: {cif, orderId},
          targetUrl: targetUrlFourPointFive,
          type: 'post',
        };
        setTimeout(
          () =>
            api
              .apiGeneral(payloadLast, dispatch)
              .then(res => {
                const email = result(res, 'data.data.email', '');
                const docSendStatus = result(
                  res,
                  'data.data.docSendStatus',
                  '',
                ).toString();
                ...
                if (docSendStatus === '2') {
                  ...
                  setTimeout(() => {
                    dispatch(
                      sendTrackingFourPointEight(email, protocolNo, orderId),
                    );
                  }, 3000);
                }
              })
  ...
  return (dispatch, getState) => {
    ...
    return api
      ...
      .then(res => {
        ...
        const targetUrlFourPointFive = '4.5';
        const payloadLast = {
          requestData: {cif, orderId, reSend},
          targetUrl: targetUrlFourPointFive,
          type: 'post',
        };
        setTimeout(
...
export function sendTrackingFourPointEight(
  email = '',
  protocolNumber = '',
  orderId = '',
) {
  ...
  if (ENV === 'dev') {
    targetUrl =
      'https://digitalloantest.banksinarmas.com:9022/sign/signPageLoad?email=' +
      email +
      '&docNo=' +
      result(protocolNumber, 'protocolNum', '');
  } else {
    targetUrl =
      'http://digitalloan.banksinarmas.com:9022/sign/signPageLoad?email=' +
      email +
      '&docNo=' +
      result(protocolNumber, 'protocolNum', '');
  }
  return (dispatch, getState) => {
    ...
    return api
      ...
      .then(res => {
        ...
        dispatch(
          NavigationActions.navigate({
            routeName: 'SigningWebViewPage',
            params: {urlAndroid: response, orderId, urlIos: targetUrl},
          }),
        );
      })
      .catch(() => {
    ...
    return api
      ...
      .then(response => {
        ...
        const checkingStatus =
          status === 'SUCCESS_REPAY' || status === 'UN_BORROW' ? '1' : '0';
        const analisis = size(responseDataRaw);
        if (analisis !== 0 && checkingStatus === '0') {
          dispatch(
            actionCreators.saveLoanAccounts([
              {...responseDataRaw, typeOfLoan: 'PGO', accountType: 'ListLoan'},
              ...dataLoan,
            ]),
          );
      ...
      return api
        ...
        .then(response => {
          ...
          if (responseCode === '00') {
            Toast.show(
              getErrorMessage(
                'Silahkan datang ke cabang terdekat unntuk melakukan pendataan lebih lanjut',
              ),
            );
            dispatch(actionCreators.hideSpinner());
          } else {
            Toast.show(
              getErrorMessage(
                'Server Time Out, silahkan dicoba beberapa saat lagi',
              ),
            );
            dispatch(actionCreators.hideSpinner());
          }
        ...
        .catch(err => {
          dispatch(actionCreators.hideSpinner());
          Toast.show(
            getErrorMessage(err, 'Tidak bisa megambil data dari server'),
          );
        });
    } else {
      dispatch(
