...
export function luckyDipSendDataAddress(pathRoute, reward, transRefNum) {
  return (dispatch, getState) => {
    ...
    const formValuesAddress = result(
      state,
      'form.LuckyDipInformation.values',
      {},
    );
    ...
    return api
      .saveAddressUser(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const resultPushOrderCode = result(
          res,
          'data.resultPushOrder.responseCode',
          '',
        );
        const resultSaveAddressCode = result(
          res,
          'data.resultSaveAddress.responseCode',
          '',
        );
        const resultUpdateOrderStatusCode = result(
          res,
          'data.resultUpdateOrderStatus.responseCode',
          '',
        );
        if (
          resultPushOrderCode === '00' &&
          resultSaveAddressCode === '00' &&
          resultUpdateOrderStatusCode === '00'
        ) {
          ...
          Toast.show(
            'Data tidak berhasil dikirim, cobalah beberapa saat lagi!',
          );
        }
      })
      .catch(() => {
        dispatch(actionCreators.hideSpinner());
        Toast.show(
          'Koneksi ke server sedang bermasalah, cobalah beberapa saat lagi!',
        );
      });
  };
}
  ...
  return dispatch => {
    getLastAddressLuckyDip().then(res => {
      ...
      if (name !== '' && phoneNumber !== '') {
        ...
        dispatch(
          change('LuckyDipInformation', 'city', result(res, 'city', {})),
        );
        dispatch(
          change(
            'LuckyDipInformation',
        ...
        dispatch(
          change('LuckyDipInformation', 'note', result(res, 'note', '')),
        );
      }
    });
  };
      ...
      .then(res => {
        ...
        dispatch(
          NavigationActions.navigate({
            ...
            params: {pathRoute, listPrize: searchEvoucher},
          }),
        );
      })
      .catch(() => {
...
export function trackingHistory(values, reward, shipmentID) {
  return dispatch => {
    ...
    return api
      .getTrackingNumber(payload, dispatch)
      .then(res => {
        const trackingNumber = result(
          res,
          'data.resultTrackingShipment.awbNo',
          '',
        );
        dispatch(actionCreators.hideSpinner());
        dispatch(
          NavigationActions.navigate({
            ...
            params: {buttonOff: true, values, reward, trackingNumber},
          }),
        );
      })
      .catch(() => {
...
export function getLuckyDipTicket() {
  return (dispatch, getState) => {
    const state = getState();
    const luckyDipMultiTierFlag =
      lowerCase(result(state, 'config.flag.flagHHHMultiTier', 'inactive')) ===
      'active';
    if (luckyDipMultiTierFlag) {
      return api
        .getCurrentTokenNew({}, dispatch)
        .then(res => {
          ...
          const currentTokenGold = String(
            result(listTokenGold, 'userToken.redeemCounter', '0'),
          );
          const tokenIdGold = String(
            result(listTokenGold, 'userToken.id', '0'),
          );
          // platinnum
          const listTokenPlatinum = find(tokenDataObjectRaw, function (val) {
            return (
              result(val, 'userToken.voucher.code', '') === 'LUCKYDIPPLATINUM'
            );
          });
          const currentTokenPlatinum = String(
            result(listTokenPlatinum, 'userToken.redeemCounter', '0'),
          );
          const tokenIdPlatinum = String(
            result(listTokenPlatinum, 'userToken.id', '0'),
          );
          // silver
          const listTokenSilver = find(tokenDataObjectRaw, function (val) {
            return (
              result(val, 'userToken.voucher.code', '') === 'LUCKYDIPSILVER'
            );
          });
          const currentTokenSilver = String(
            result(listTokenSilver, 'userToken.redeemCounter', '0'),
          );
          const tokenIdSilver = String(
            result(listTokenSilver, 'userToken.id', '0'),
          );
          const currentToken = String(
            Number(currentTokenPlatinum) +
              Number(currentTokenGold) +
              Number(currentTokenSilver),
          );
          ...
          const formatTimeserver =
            moment(timeServerCurrent).format('YYYY/MM/DD H:mm');
          const differenceTime = moment(localTime).diff(
            moment(formatTimeserver),
          );
          const gapTime = Math.round(
            moment.duration(differenceTime).asSeconds(),
          );
          const rawnextDateServer =
            moment(timeServerCurrent).format('YYYY/MM/DD');
          const nextDate = rawnextDateServer + ' ' + '23:59';
          const diff = moment(nextDate).diff(moment(formatTimeserver));
          const gapTimeEndedServer = Math.round(
            moment.duration(diff).asSeconds(),
          );
          const payloadCacheTicket = {
            localTime,
            timeServerCurrent,
            formatTimeserver,
            tokenId: {tokenIdGold, tokenIdPlatinum, tokenIdSilver},
            gaptimeServeLocal: gapTime,
            currentToken,
            gapTimeEndedServer,
            totalGapLocalServer: gapTimeEndedServer + gapTime,
          };
          set(storageKeys.LUCKY_DIP_TICKET, payloadCacheTicket);
          dispatch(actionCreators.getSaveLuckyDipBox(payloadCacheTicket));
          dispatch(
            actionCreators.saveCounterLuckydip({
              currentToken,
              tokenId: {tokenIdGold, tokenIdPlatinum, tokenIdSilver},
              currentTokenGold: currentTokenGold,
              currentTokenPlatinum: currentTokenPlatinum,
              currentTokenSilver: currentTokenSilver,
            }),
          );
        })
        .catch(() => {});
      ...
      return api
        .getCurrentToken({}, dispatch)
        .then(res => {
          const state = getState();
          const tokenDataObjectRaw = result(res, 'data.userToken', {});
          const currentToken = String(
            result(tokenDataObjectRaw, 'redeemCounter', '0'),
          );
...
export function getProvinceList() {
  return (dispatch, getState) => {
    ...
    if (isEmpty(provinceListReducer)) {
      ...
      return api
        .getLocationAlfaTrex(payload, dispatch)
        .then(res => {
