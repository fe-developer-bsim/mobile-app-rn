...
export function setAirport(airport, navigation) {
  return (dispatch, getState) => {
    ...
    const originCity = result(
      state,
      'form.FlightIndexForm.values.originCity',
      '',
    );
    const destinationCity = result(
      state,
      'form.FlightIndexForm.values.destinationCity',
      '',
    );

    const codeOrigin = originCity.substring(
      originCity.indexOf('[') + 1,
      originCity.indexOf(']'),
    );
    const codeDestination = destinationCity.substring(
      destinationCity.indexOf('[') + 1,
...
export function getFlightAvailability(adult, child, infant, inquiry) {
  return (dispatch, getState) => {
    ...
    const Origin = originCity.substring(
      originCity.indexOf('[') + 1,
      originCity.indexOf(']'),
    );
    const Destination = destinationCity.substring(
      destinationCity.indexOf('[') + 1,
      destinationCity.indexOf(']'),
    );

    const departDateRaw = result(
      state,
      'form.FlightIndexForm.values.departDate',
      '',
    );
    const DepartDate =
      departDateRaw === '' ? '' : moment(departDateRaw).format('YYYY-MM-DD');
    const arrivalDateRaw = result(
      state,
      'form.FlightIndexForm.values.arrivalDate',
      '',
    );
    const ReturnDate =
      arrivalDateRaw === '' ? '' : moment(arrivalDateRaw).format('YYYY-MM-DD');
    ...
    if (ReturnDate === '') {
      ...
      return api
        .getFlightAvailability(payload, dispatch)
        .then(res => {
          dispatch(actionCreators.hideSpinner());
          if (
            res.data.responseCode === '00' &&
            res.data.getFlightAvailabilityMap[0].Flights.length > 0 &&
            res.data.getFlightAvailabilityMap.length > 0
          ) {
            if (inquiry === 'new') {
              dispatch(
                NavigationActions.navigate({routeName: 'FlightSchedule1'}),
              );
              dispatch(
                actionCreators.saveFlightSchedule(
                  res.data.getFlightAvailabilityMap,
                ),
              );
        ...
        .catch(err => {
          Toast.show(
            getErrorMessage(err, language.FLIGHT__ERR_MSG1),
            Toast.LONG,
          );
          dispatch(actionCreators.hideSpinner());
        });
      ...
      return api
        ...
        .then(res => {
          ...
          if (
            res.data.responseCode === '00' &&
            res.data.getFlightAvailabilityMap[0].Flights.length > 0 &&
            res.data.getFlightAvailabilityMap[1].Flights.length > 0 &&
            res.data.getFlightAvailabilityMap.length > 0
          ) {
            if (inquiry === 'new') {
              dispatch(
...
export function getFareDetail(item) {
  return (dispatch, getState) => {
    ...
    dispatch(actionCreators.showSpinner());
    return api
      .getFareDetail(payload, dispatch)
      .then(res => {
        if (
          res.data.responseCode === '00' &&
          res.data.getFareDetailMap !== null
        ) {
          dispatch(actionCreators.hideSpinner());
          dispatch(
            NavigationActions.navigate({
              routeName: 'FlightDetail',
              params: {data: res.data.getFareDetailMap, flag: flag},
            }),
          );
        }
      })
...
export function txTravelDetailList(toDetailList, hitApi) {
  return (dispatch, getState) => {
    ...
    if (hitApi) {
      api
        .countryIso(payload, dispatch)
        .then(res => {
          dispatch(actionCreators.saveCountryIso(res));
        })
        .catch(err => {
          Toast.show(
            getErrorMessage(err, language.TX_TRAVEL_FAIL_GET_COUNTRY_ISO),
            Toast.LONG,
          );
        });

      api
        .getListPassenger(payload, dispatch)
        .then(res => {
        ...
        .catch(err => {
          Toast.show(
            getErrorMessage(err, language.TX_TRAVEL_FAIL_GET_PASSENGER),
            Toast.LONG,
          );
          dispatch(actionCreators.hideSpinner());
        });
...
export function txTravelDetail(index, type, isInternational) {
  return (dispatch, getState) => {
    ...
    dispatch(
      NavigationActions.navigate({
        routeName: 'TxTravelDetail',
        params: {index, type, isInternational, state},
      }),
    );
  };
}
  ...
  return (dispatch, getState) => {
    ...
    return api
      ...
      .then(res => {
        ...
        dispatch(
          NavigationActions.navigate({
            routeName: 'TxTravelHistory',
            params: {res: res, tipe: tipe, state},
          }),
        );
      })
      .catch(err => {
...
export function fareDetail() {
  return (dispatch, getState) => {
    ...
    const flightData1 = prepareSaveBooking(
      passegerDetail,
      flightDataDetail1,
      'departure',
      state,
    ...
    if (flagWay !== 'oneWay') {
      ...
      flightData2 = prepareSaveBooking(
        passegerDetail,
        flightDataDetail2,
        'arrival',
        state,
    ...
    return api
      .getFareDetail(payload1, dispatch)
      .then(res => {
        ...
        if (flagWay === 'oneWay') {
          ...
          return api
            .getFareDetail(payload2, dispatch)
            .then(res2 => {
              ...
              const isDifferentAirlines =
                parseInt(result(segment1[0], 'Airline', 0)) ===
                parseInt(result(segment2[0], 'Airline', 0));
              flightData = {
                ...flightData,
                isDifferentAirlines: !isDifferentAirlines,
              };
              if (
                res2.data.responseCode === '00' &&
                res2.data.getFareDetailMap !== null
              ) {
                dispatch(
                  actionCreators.saveFareDetail2(res2.data.getFareDetailMap),
                );
                const hideAlert = () => {
                  dispatch(actionCreators.hideSinarmasAlert());
                  dispatch(
            ...
            .catch(err => {
              Toast.show(
                getErrorMessage(err, language.TX_TRAVEL_NO_FLIGHT_AVAILABLE),
                Toast.LONG,
              );
              dispatch(actionCreators.hideSpinner());
            });
...
export function txTravelPassenger(data, tipe) {
  return (dispatch, getState) => {
    ...
    return api
      .addListPassenger(payload, dispatch)
      .then(() => {})
      .catch(err => {
        Toast.show(
          getErrorMessage(err, language.TX_TRAVEL_FAIL_ADD_PASSENGER),
          Toast.LONG,
        );
        dispatch(actionCreators.hideSpinner());
      });
...
export function goTx(emallData) {
  return (dispatch, getState) => {
    ...
    return api
      .getFlightReserv(payload, dispatch)
      .then(res => {
        ...
        if (startsWith(cifCode, 'NK')) {
          ...
          dispatch(
            NavigationActions.navigate({
              routeName: 'EmallRoutes',
              params: {...emallData, reservData, totalAmount},
            }),
          );
        } else {
          if (
      ...
      .catch(err => {
        dispatch(actionCreators.hideSpinner());
        Toast.show(
          getErrorMessage(err, language.ERROR_MESSAGE__FLIGHT_RESERV),
          Toast.LONG,
        );
      });
  };
...
export function getFlightResult(
  ...
  return (dispatch, getState) => {
    const onSubmit = () => {
      ...
      dispatch(actionCreators.showSpinner());
      return api
        .getFlightPayment(payload, additional, dispatch)
        .then(res => {
          dispatch(actionCreators.hideSpinner());
...
export function getConfirmInfo(
  ...
  return dispatch => {
    dispatch(
      NavigationActions.navigate({
        routeName: 'EmallTxInfo',
        params: {flightData, accData, reservData, segmentsReturn},
      }),
    );
  };
}
