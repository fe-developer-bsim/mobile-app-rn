...
export const getCityList =
  (fieldName, onClearChange) => (dispatch, getState) => {
    const state = getState();
...
export const getSubDistrictList =
  (fieldName, onClearChange) => (dispatch, getState) => {
    const state = getState();
    const formName = 'DigitalEForm';
    const formValues = state.form[`${formName}`].values;
        ...
        if (response !== 'granted') {
          Permissions.request('ios.permission.CAMERA').then(response => {
            if (response !== 'granted') {
              dispatch(actionCreators.hideSpinner());
              Toast.show(
                'Go to settings, allow SimobiPlus to access device camera',
                Toast.LONG,
              );
              return Promise.resolve();
            } else {
...
export function getCreditCardProductsItems() {
  return (dispatch, getState) => {
    ...
    const payload = {ipassport: newIpass};
    api
      .getCreditCardProducts(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const config = result(res, 'data.config', []);
        ...
        dispatch(
          actionCreators.saveProductsItem({config, configTitle, isCreditCard}),
        );
        dispatch(NavigationActions.navigate({routeName: 'ChooseProductsItem'}));
      })
      .catch(err => {
      ...
      if (productCode.includes('SA')) {
        if (startsWith(cifCode, 'NK')) {
          ...
          if (emoneyKycOnly) {
            ...
            dispatch(
              NavigationActions.navigate({routeName: 'SavingEmailForm'}),
            );
          }
        }
      } else {
...
export function getCurrentSection(isOnboard = false) {
  return (dispatch, getState) => {
    ...
    const mobileNumber = formatMobileNumberEmoneyRegistration(
      emoneyNoAcc.substring(2, emoneyNoAcc.length),
    );
      ...
      .then(res => {
        ...
        if (responseCode === '00') {
          if (isCheckLimit) {
            ...
            if (isOnboard) {
              dispatch(
                actionCreators.saveCurrentSection(
...
export function checkLiveness(params) {
  return (dispatch, getState) => {
    ...
    let adjustEvent;
    if (productCode.includes('SA')) {
      // event adjust tracking selfie for liveness
      if (Platform.OS === 'android') {
        adjustEvent = new adjustAndroid.AdjustEvent('csndco');
      ...
      NativeModules.LivenessModule.startLiveness(
        (livenessId, base64Image, transactionId) => {
          ...
          if (pageName === 'camera') {
            ...
            const field =
              filter(fields, obj => obj.component === 'selfieCamera')[0] ||
              fields[0]; // fields that uses camera or the first field if none specified
            ...
            const formValues = result(
              getState(),
              `form.${eFormFields.formName}.values`,
              {},
            );
            dispatch(
    ...
    return api
      ...
      .then(res => {
        ...
        if (isGoToSection) {
          dispatch(destroy('DigitalEForm'));
          dispatch(
            actionCreators.saveCurrentSection(result(res.data, 'config', [])),
          );
          if (productCode === 'PD') {
            dispatch(
              StackActions.reset({
            ...
            dispatch(
              StackActions.reset({
                ...
                actions: [
                  NavigationActions.navigate({routeName: 'Landing'}),
                  NavigationActions.navigate({
                    routeName: 'EmoneyUpgradeBenefit',
                  }),
                  NavigationActions.navigate({routeName: 'CurrentSection'}),
                ],
              }),
          ...
          const params = {item, pageName, pageCode};
          dispatch(actionCreators.saveEForm(storedData));
...
export function checkKtpDukcapil(fieldName, pageName, data, isKtp = false) {
  return (dispatch, getState) => {
    ...
    const dob = moment(
      result(state, 'form.DigitalEForm.values.0~1~6', ''),
    ).format('YYYY-MM-DD');
...
export function generatePhoto(fieldName, pageName, data) {
  return (dispatch, getState) => {
    ...
    return api
      .eformGeneralNew(payload, dispatch)
      .then(res => {
    ...
    return api
      ...
      .then(res => {
        ...
        if (isCommit) {
          ...
          dispatch(
            actionCreators.saveCurrentSection(result(nextForm, 'config', [])),
          );
          if (productCode === 'PD') {
...
const set =
  ({fieldName, formValues, dependentOn, dependentOnKeyword = 'option'}) =>
  dispatch => {
    const value = result(
      formValues,
      `${dependentOn}.${dependentOnKeyword}`,
      '',
    );
    value &&
      formValues[fieldName] !== value &&
      dispatch(change(eFormFields.formName, fieldName, value));
  };
...
const setZipcode =
  ({
    ...
    formValues,
    dependentOn,
    dependentOnKeywordZipcode = 'option',
  }) =>
  dispatch => {
    const value = result(
      formValues,
      `${dependentOn}.${dependentOnKeywordZipcode}`,
      '',
    );
    value &&
      formValues[fieldName] !== value &&
...
export function checkPhoneNumber(firebaseEmoney) {
  return (dispatch, getState) => {
    ...
    const mobileNumber = formatMobileNumberEmoneyRegistration(
      result(state, 'form.EmoneyRegistrationForm.values.phone', ''),
    );
    const captchaInput = result(
      state,
      'form.EmoneyRegistrationForm.values.captchaInput',
      '',
    );
    ...
    let adjustEvent;
    if (productCode.includes('SA')) {
      // event adjust tracking registration
      if (Platform.OS === 'android') {
        adjustEvent = new adjustAndroid.AdjustEvent('csndco');
    ...
    return api
      ...
      .then(res => {
        ...
        if (responseCode === '00') {
          dispatch(
            NavigationActions.navigate({
              routeName: 'DigitalEFormEmailVerification',
              params: {pageName},
            }),
          );
        }
    ...
    return api
      ...
      .then(res => {
        ...
        if (responseCode === '00') {
          dispatch(
            NavigationActions.navigate({
              routeName: 'DigitalEFormSuccessScreen',
              params: {data},
            }),
          );
        }
...
export function showRoughLimitModal(res) {
  return dispatch => {
    ...
    const sinarmasModalOptions = {
      ...
      text:
        language.EFORM__LIMIT_KEY_1 +
        language.EFORM__LIMIT_KEY_2 +
        currencyFormatter(result(res, 'data.nextForm.limit', '')),
...
export function onPeriodChange() {
  return (dispatch, getState) => {
    const state = getState();
    const isNewApply = result(
      state,
      'form.DigitalEForm.values.isNewApply',
      true,
    );
    if (isNewApply === false) {
...
export function getListLoanProduct() {
  return (dispatch, getState) => {
    ...
    const payload = {ipassport: newIpass};
    api
      .getLoanProducts(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const config = result(res, 'data.config', []);
  ...
  return (dispatch, getState) => {
    ...
    return api
      ...
      .then(res => {
        ...
        const statNum =
          status !== '99' && status !== '98' && status !== '97' ? status : null;
...
export const verificationPopUp = isAdvanceAI => dispatch => {
  const goToVerif = () => {
    ...
    if (isAdvanceAI) {
      ...
      dispatch(
        NavigationActions.navigate({
          routeName: 'ConfirmEditProfileSelfieCamera',
        }),
      );
    }
  };
...
export function commitConfirmEditProfile() {
  return (dispatch, getState) => {
    ...
    return api
      .commitPengkinianData(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const responseCode = result(res, 'data.responseCode', '');
        ...
        if (responseCode === '00') {
          dispatch(actionCreators.saveConfirmFields(data));
          dispatch(
            NavigationActions.navigate({routeName: 'SuccessVerification'}),
          );
        }
      })
      .catch(error => {
...
export function filterListIndustry(fieldName, onClearChange) {
  return (dispatch, getState) => {
    ...
    let listIndustryNew = result(
      state,
      'configEForm.listConfigEform.simplifiedIndustryList',
      [],
    );
    ...
    if (name === '') {
      ...
      dispatch(
        actionCreators.saveSimplifiedListIndustryNew(filteredListIndustryNew),
      );
      dispatch(clearData(onClearChange));
    }
  };
...
export function filterListJob() {
  return (dispatch, getState) => {
    const state = getState();
    const productCode = result(state, 'productCode', '');
    const simplifiedJobList = result(
      state,
      'configEForm.listConfigEform.simplifiedJobList',
      [],
    );
    ...
    dispatch(
      actionCreators.saveSimplifiedListJobNew(sortedUniq(newSimplifiedJobList)),
    );
  };
}
...
export function getLivenessIdIOS(pageCode, livenessId) {
  return (dispatch, getState) => {
    ...
    if (productCode === 'PD') {
      ...
      const fields = result(page, 'fields', []);
      const field =
        filter(fields, obj => obj.component === 'selfieCamera')[0] || fields[0]; // fields that uses camera or the first field if none specified
      const {code: fieldName} = field;
      dispatch(change(eFormFields.formName, fieldName, livenessId)); // 0~1~3
      const formValues = result(
        getState(),
...
export function goToTnC(value) {
  return (dispatch, getState) => {
    ...
    const loginRoute = result(nav, 'routes.0.routeName', 'Onboarding');
    const isUserLogin =
      isLogin && (result(nav, 'index', 0) > 0 || loginRoute === 'Main');
    const emoneyKycOnly =
      getEmoneyKyc(accountList).length === 1 && !startsWith(cifCode, 'NK');
