...
import {
  ...
  getCurrentRoute,
  maskedEmail as mask,
} from '../../utils/transformer.util.js';
...
export const sendEmailOTP = email => (dispatch, getState) => {
  ...
  return get(storageKeys.LANGUAGE)
    .then(lang => api.generateEmailOTP({email, lang}, dispatch))
    .then(() => {
      dispatch(actionCreators.hideSpinner());
      tracker.trackEvent('RESEND_OTP', 'VERIF_EMAIL', null, {
        label: `userId:${userId}`,
      });
    })
    .catch(err => {
      ...
      return Promise.reject(
        getErrorMessage(err, language.EMAIL_OTP__CANNOT_SEND_OTP),
      );
    });
};
...
export const verifyEmailOTP = otpCode => dispatch => {
  ...
  dispatch(actionCreators.showSpinner());
  return api
    .validateEmailOTP(verifyEmailOTPPayload, dispatch)
    .then(res => {
      ...
      return responseCode === '00'
        ? Promise.resolve()
        : Promise.reject(
            getErrorMessage(res, language.ERROR_MESSAGE__UNKNOWN_ERROR),
          );
    })
    .catch(err => {
      dispatch(actionCreators.hideSpinner());
