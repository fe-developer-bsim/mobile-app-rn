...
export const GenflixUsernameQuery = username => dispatch => {
  ...
  return api
    .genflixQuery(queryPayload, dispatch)
    .then(() => Promise.resolve({isUser: true}))
    .catch(err => {
      // 93 = MEMBER_NOT_FOUND
      const responseCode = result(err, 'data.responseCode', {});
      return responseCode === '93'
        ? Promise.resolve({isUser: false})
        : Promise.reject(
            getErrorMessage(err, language.ERROR_MESSAGE__UNKNOWN_ERROR),
          );
    });
};

export const GenflixUsernameRegistration =
  (username, password, accountInfo) => (dispatch, getState) => {
    ...
    const genflixRegistrationPayload = {
      easyPin,
      smsOtp,
      simasToken,
      email: username,
    ...
    return get(storageKeys.LANGUAGE)
      .then(lang =>
        api
          .genflixRegistration({...genflixRegistrationPayload, lang}, dispatch)
          .then(res => {
            Toast.show(
              `${language.GENFLIX__REGISTRATION_MESSAGE} ${username} ${language.GENFLIX__REGISTRATION_SUCCESSFUL}`,
              Toast.LONG,
            );
            ...
            Toast.show(
              `${language.GENFLIX__REGISTRATION_MESSAGE} ${email} ${language.GENFLIX__REGISTRATION_SUCCESSFUL}`,
              Toast.LONG,
            );
            ...
            const dataDetail = [
              {key: 'GENFLIX__SUBSCRIPTION_ID', value: orderId},
              {key: 'GENFLIX__DATE', value: date},
            ];
...
export const GenflixPaymentConfirmation =
  (username, password, account) => dispatch => {
...
export const GenflixResult = resData => (dispatch, getState) => {
  ...
  dispatch(actionCreators.showSpinner());
  return dispatch(GenflixUsernameRegistration(username, password, account))
    .then(res => {
      modalOptions = {
        ...modalOptions,
        heading: result(res, 'data.order_id', ''),
      };
      dispatch(actionCreators.hideSpinner());
      ...
      dispatch(actionCreators.hideSpinner());
    }, dispatch(handleTxError({modalOptions})))
    .catch(handleTxError({modalOptions}));
};
...
const handleTxError = props => dispatch => err => {
  Toast.show(
    getErrorMessage(
      err.data,
      language.ERROR_MESSAGE__COULD_NOT_FINALIZE_AUTODEBIT,
    ),
  );
