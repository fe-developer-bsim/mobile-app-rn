...
export function inquiryProxyByEDW() {
  return dispatch => {
    ...
    const payload = '';
    return api
      .inquiryProxyByEDW(payload, dispatch)
      .then(res => {
      ...
      .then(() =>
        api
          .detailByCustNo(payload, dispatch)
          .then(res => {
...
export function goClaimRewardNow(
  ...
  return (dispatch, getState) => {
    ...
    const responseMessageHeader = result(
      state,
      'proxyResolution.responseMessageHeader',
      '',
    );
    const responseMessageTitle = result(
      state,
      'proxyResolution.responseMessageTitle',
    );
    const hideAlert = () => {
      dispatch(actionCreators.hideSinarmasAlert());
    };
    const gotoHomeLock = () => {
      dispatch(
        NavigationActions.navigate({
          routeName: 'NewProxyConfirmationBIFast',
          params: {addProxyAddress, proxyAlias, myAccount, responseCode},
        }),
      );
      dispatch(actionCreators.hideSinarmasAlert());
    };
...
export function goToConfirmationParams(addProxyAddress, proxyAlias, myAccount) {
  return (dispatch, getState) => {
    ...
    const BICCustomer = result(
      state,
      'detailByCustNo.dataResolution.BICCustomer',
      '',
    );
    const proxyRequestType = result(
      state,
      'detailByCustNo.dataResolution.proxyResolution',
      '',
    );
    const proxyType = result(
      state,
      'form.addProxyAddress.values.addProxyType.value',
    );
    ...
    return api
      .proxyResolution(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const detailByCustNoData = result(res, 'data', []);
        ...
        if (responseCode === '91') {
          ...
          dispatch(
            NavigationActions.navigate({
              routeName: 'NewProxyConfirmationBIFast',
              params: {addProxyAddress, proxyAlias, myAccount},
            }),
          );
        }
      })
...
export function addRegisterProxyAddress(data) {
  return (dispatch, getState) => {
    ...
    const AccountLWIdentification_Id = result(
      data,
      'myAccount.accountNumber',
      '',
    );
    const ProxyRegistration1_RegnTp = result(
      state,
      'detailByCustNo.dataProxy.proxyTypeRegister',
      '',
    );
    const ProxyDefinition1_Tp = result(
      state,
      'form.addProxyAddress.values.addProxyType.value',
    );
    const ProxyDefinition1_Val = result(data, 'proxyAlias', '');
    const ProxyRegistrationAccount1_DsplNm = result(
      state,
      'insuranceDataTravel.name',
      '',
    );
    const GenericFinancialIdentification1_Id = result(
      state,
      'detailByCustNo.dataResolution.BICCustomer',
      '',
    );
    const GenericAccountIdentification1_Id = result(
      data,
      'myAccount.accountNumber',
      '',
    );
    const CashAccountType2ChoiceProxy_Prtry = result(
      data,
      'myAccount.accountType',
    );
    const CashAccount40_Nm = result(data, 'myAccount.name', '');
    const ScndIdDefinition1_Tp = result(
      state,
      'detailByCustNo.responseGetDetailByCustNo.body.response.customerIdTypes',
    );
    const ScndIdDefinition1_Val = result(
      state,
      'detailByCustNo.responseGetDetailByCustNo.body.response.legalIdNumber',
    );
    const ProxyRegistrationAccount1_RegnSts = result(
      state,
      'detailByCustNo.dataProxy.proxyTypeActiveByCust',
    );
    const BI_AddtlCstmrInf_Tp = result(
      state,
      'detailByCustNo.responseGetDetailByCustNo.body.response.customerType',
    );
    const BI_AddtlCstmrInf_Id = result(
      state,
      'detailByCustNo.responseGetDetailByCustNo.body.response.legalIdNumber',
    );
    const BI_AddtlCstmrInf_RsdntSts = result(
      state,
      'detailByCustNo.responseGetDetailByCustNo.body.response.residence',
    );
    const BI_AddtlCstmrInf_TwnNm = result(
      state,
      'detailByCustNo.responseGetDetailByCustNo.body.response.locationCode',
    );
    const payload = {
      easyPin,
      transRefNum,
      ...
      ProxyDefinition1_Tp,
      ProxyDefinition1_Val,
      ProxyRegistrationAccount1_DsplNm,
      GenericFinancialIdentification1_Id,
      GenericAccountIdentification1_Id,
      CashAccountType2ChoiceProxy_Prtry,
      CashAccount40_Nm,
      ScndIdDefinition1_Tp,
      ScndIdDefinition1_Val,
      ProxyRegistrationAccount1_RegnSts,
      BI_AddtlCstmrInf_Tp,
      BI_AddtlCstmrInf_Id,
      BI_AddtlCstmrInf_RsdntSts,
      BI_AddtlCstmrInf_TwnNm,
    };
    return api
      .proxyRegister(payload, dispatch)
      .then(res => {
...
export function portingRegisterProxyAddress(data) {
  return (dispatch, getState) => {
    ...
    const ProxyRegistration1_RegnTp = result(
      state,
      'detailByCustNo.dataProxy.proxyTypePorting',
      '',
    );
    const ProxyDefinition1_Tp = result(
      state,
      'proxyResolution.responseResolution.body.response.ProxyDefinition1_Orgnl_Tp',
      '',
    );
    const ProxyDefinition1_Val = result(
      state,
      'proxyResolution.responseResolution.body.response.ProxyDefinition1_Orgnl_Val',
      '',
    );
    const ProxyRegistrationAccount1_RegnId = result(
      state,
      'proxyResolution.responseResolution.body.response.ProxyLookUpAccount1_RegnId',
      '',
    );
    const ProxyRegistrationAccount1_DsplNm = result(
      state,
      'proxyResolution.responseResolution.body.response.ProxyLookUpAccount1_DsplNm',
      '',
    );
    const GenericFinancialIdentification1_Id = result(
      state,
      'proxyResolution.responseResolution.body.response.GenericFinancialIdentification1_Id',
      '',
    );
    const GenericAccountIdentification1_Id = result(
      data,
    ...
    const CashAccountType2ChoiceProxy_Prtry = result(
      state,
      'proxyResolution.responseResolution.body.response.CashAccountType2ChoiceProxy_Prtry',
    );
    const CashAccount40_Nm = result(
      state,
      'proxyResolution.responseResolution.body.response.CashAccount40_Nm',
      '',
    );
    const ScndIdDefinition1_Tp = result(
      state,
    ...
    const payload = {
      ...
      ProxyDefinition1_Tp,
      ProxyDefinition1_Val,
      ProxyRegistrationAccount1_RegnId,
      ProxyRegistrationAccount1_DsplNm,
      GenericFinancialIdentification1_Id,
      GenericAccountIdentification1_Id,
      CashAccountType2ChoiceProxy_Prtry,
...
export function inquiryProxyAddress() {
  return (dispatch, getState) => {
    ...
    const accountCustomer = result(
      state,
      'form.transferProxy.values.myAccount.accountNumber',
      '',
    );
    const proxyValue = upperCase(
      result(state, 'form.ProxyAddressTrf.values.proxyAddress', ''),
    );
    ...
    return api
      .proxyResolutionCT(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        ...
        const GenericFinancialIdentification1_Id = result(
          res,
          'data.responseResolution.body.targetBankName',
        );
        const CashAccount40_Nm =
          result(
            res,
            'data.responseResolution.body.response.CashAccount40_Nm',
          ) +
          ' - ' +
          result(
            res,
            'data.responseResolution.body.response.GenericAccountIdentification1_Id',
          );
        const isGenericFinancialIdentification1_Id = isEmpty(
          GenericFinancialIdentification1_Id,
        )
          ? ''
          : GenericFinancialIdentification1_Id;
        const isCashAccount40_Nm =
          CashAccount40_Nm === 'undefined - undefined' ? '' : CashAccount40_Nm;
        dispatch(
          change(
            'ProxyAddressTrf',
            'bankNameCode',
            isGenericFinancialIdentification1_Id,
          ),
        );
        dispatch(
          change('ProxyAddressTrf', 'accountDetail', isCashAccount40_Nm),
        );
      })
      .catch(err => {
        dispatch(actionCreators.hideSpinner());
...
export function goToEditConfirmation(
  formValues,
  user,
  detailByCustNo,
  selectedAccount,
  ...
  return dispatch => {
    dispatch(
      NavigationActions.navigate({
        routeName: 'EditProxyConfirmationBIFast',
        params: {formValues, user, detailByCustNo, selectedAccount, myAccount},
      }),
    );
  };
}
...
export function proxyUpdate() {
  return (dispatch, getState) => {
    ...
    const selectionAcc = omit(allValueData, [
      'GenericAccountIdentification1_Id',
    ]);
    let easyPin = result(state, 'form.AuthenticateForm.values.easypin', '');
    // const mPinInputed = result(state, 'form.AuthenticateForm.values.otp', '');
    ...
    const accountNumber = result(
      state,
      'form.BifastEditMenu.values.myAccount.accountNumber',
      '',
    );
    ...
    const ProxyRegistration1_RegnTp = result(
      state,
      'detailByCustNo.dataProxy.proxyTypeUpdate',
      '',
    );
...
export function unlinkProxy(user, detailByCustNo, selectedAccount, formValues) {
  return (dispatch, getState) => {
    ...
    const goUnlink = () => {
      ...
      const ProxyRegistration1_RegnTp = result(
        state,
        'detailByCustNo.dataProxy.proxyTypeDeregister',
        '',
      );
        ...
        .catch(err => {
          ...
          if (easyPinAttempt === 'invalid') {
            ...
            Toast.show(
              getErrorMessage(
                err,
                language.ERROR_MESSAGE_INVALID_EASYPIN_TRANSACTION,
              ),
              Toast.LONG,
            );
          } else if (easyPinAttempt === 'blocked') {
