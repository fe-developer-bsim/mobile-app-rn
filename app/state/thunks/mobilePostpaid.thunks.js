...
export function getAmountForPostpaid(mobileNo, selectedBiller) {
  return dispatch => {
    const payload = middlewareUtils.preparePostpaidEnquiryPayload(
      mobileNo,
      selectedBiller,
    );
    ...
    return api
      .enquirePostpaidBill(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        dispatch(
          NavigationActions.navigate({
            routeName: 'MobilePostpaidPayment',
            params: {
      ...
      .catch(err => {
        ...
        const defaultMessage =
          language.ERROR_MESSAGE__COULD_NOT_GET_POSTPAID_AMOUNT;
...
export function payPostpaidBill(biller, billDetails, accountFrom) {
  return (dispatch, getState) => {
    ...
    const lastPostpaidPayment = result(
      storeState,
      'lastPostpaidPayments.allTransactions',
      '',
    );
    const transRefNum = storeState.transRefNum;
    const payloadRaw = middlewareUtils.preparePostpaidTransactionPayload({
      smsOtp,
      simasToken,
      transRefNum,
      biller,
    ...
    dispatch(actionCreators.showSpinner());
    return api
      .resultMobilePostPaid(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        ...
        dispatch(
          actionCreators.updateLastPostpaidPayments(
            middlewareUtils.getMobilePostpaidHistory({
              biller,
              billDetails,
              mobileNo,
            }),
        ...
        set(storageKeys.ALL_POSTPAID_TRANSACTIONS, lastPostpaidPayment).catch(
          err => {
            Toast.show(
              getErrorMessage(
                err,
      ...
      .catch(err => {
        ...
        if (easyPinAttempt === 'errHsm') {
          ...
          const errorText = getErrorMessage(
            err,
            language.ERROR_MESSAGE__COULD_NOT_PAY_POSTPAID_BILL,
          );
          dispatch(
            errorResponseResult(
              err,
...
export function confirmationPostpaidBill(biller, billDetails, mobileNo) {
  return (dispatch, getState) => {
    ...
    const selectedAccount = result(
      storeState,
      'form.MobilePostpaidPaymentForm.values.accountNumber',
      '',
    );
    const transRefNum = storeState.transRefNum;
    const payload = middlewareUtils.preparePostpaidTransactionPayload({
      easyPin,
      smsOtp,
      simasToken,
      transRefNum,
    ...
    return api
      .confirmationMobilePostPaid(payload, dispatch)
      .then(res => {
        ...
        dispatch(
          StackActions.reset({
            ...
            actions: [
              ...
              NavigationActions.navigate({
                routeName: 'MobilePostpaidConfirmation',
                params: {
                  confirmationDisplay,
                  mobileNo,
                  biller,
