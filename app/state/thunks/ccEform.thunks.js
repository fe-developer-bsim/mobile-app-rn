...
import {
  ...
  getFormName,
  getFieldName,
  generateCaptchaOpenProduct as generateCaptcha,
...
export function createCreditCardForm(
  ...
  return (dispatch, getState) => {
    ...
    const name =
      result(state, 'openAccountData.name', '') === ''
        ? result(state, 'user.profile.name', '')
        : result(state, 'openAccountData.name', '');
    const referralCode =
      result(state, 'openAccountData.referralCode', '') === ''
        ? result(state, 'couponCheck.referralCodeCC', '')
        : result(state, 'openAccountData.referralCode', '');
    ...
    const npwpImage =
      result(state, 'openAccountData.npwpImage', '') === ''
        ? result(state, 'form.CameraNPWPForm.values.imageData', '')
        : result(state, 'openAccountData.npwpImage', '');
    ...
    // First Form
    const ktpId =
      result(state, 'form.CCForm1.values.ktpId', '') === ''
        ? result(state, 'openAccountData.ktpId', '')
        : result(state, 'form.CCForm1.values.ktpId', '');
    const maritalStatus = result(
      state,
      'form.CCForm1.values.maritalStatus',
      '',
    );
    const dob =
      result(state, 'form.CCForm1.values.birthDate', '') === ''
        ? ''
        : moment(result(state, 'form.CCForm1.values.birthDate', '')).format(
            'YYYY-MM-DD',
          );
    const mothersMaiden = result(
      state,
      'form.CCForm1.values.mothersMaiden',
      '',
    );
    const ktpId2 = result(state, 'form.CCForm1.values.ktpId2', '');
    const maritalStatus2 = result(
      state,
      'form.CCForm1.values.maritalStatus2',
      '',
    );
    const dob2 = result(state, 'form.CCForm1.values.dob2', '');
    const mothersMaiden2 = result(
      state,
      'form.CCForm1.values.mothersMaiden2',
      '',
    );
    ...
    // Second Form
    const country = isEmpty(result(state, 'form.CCForm2.values', {}))
      ? ''
      : 'Indonesia';
    ...
    const streetAddress = result(
      state,
      'form.CCForm2.values.streetAddress',
      '',
    );

    const usingKtpData = result(
      state,
      'form.CCForm2.values.usingKtpData',
      true,
    );
    ...
    const streetAddress2 = result(
      state,
      'form.CCForm2.values.streetAddress2',
      '',
    );
    ...
    const monthlyIncome = result(
      state,
      'form.CCForm3.values.monthlyIncome',
      '',
    );
    ...
    const companyAddress = result(
      state,
      'form.CCForm4.values.companyAddress',
      '',
    );
    const companyPhoneNumber = result(
      state,
      'form.CCForm4.values.companyPhoneNumber',
      '',
    );
    const companyCountry = isEmpty(result(state, 'form.CCForm4.values', {}))
      ? ''
      : 'Indonesia';
    const companyProvince = result(
      state,
      'form.CCForm4.values.companyProvince',
      '',
    );
    const companyCity = result(state, 'form.CCForm4.values.companyCity', '');
    const companyDistrict = result(
      state,
      'form.CCForm4.values.companyDistrict',
      '',
    );
    const companySubDistrict = result(
      state,
      'form.CCForm4.values.companySubDistrict',
      '',
    );
    const companyPostalCode = result(
      state,
      'form.CCForm4.values.companyPostalCode',
      '',
    );
    const companyrt = result(state, 'form.CCForm4.values.companyRT', '');
    const companyrw = result(state, 'form.CCForm4.values.companyRW', '');
    const companyRtRw =
      companyrt === '' && companyrw === '' ? '' : companyrt + '/' + companyrw;

    // Fifth Form
    const lastEducation = result(
      state,
      'form.CCForm5.values.lastEducation',
      '',
    );
    const purposeOfOpening = result(
      state,
      'form.CCForm5.values.purposeOfOpening',
      '',
    );
    const numberOfDebit = result(
      state,
      'form.CCForm5.values.numberOfDebit',
      '',
    );
    const debitPerMonth = result(
      state,
      'form.CCForm5.values.debitPerMonth',
      '',
    );
    const numberOfCredit = result(
      state,
      'form.CCForm5.values.numberOfCredit',
      '',
    );
    const creditPerMonth = result(
      state,
      'form.CCForm5.values.creditPerMonth',
      '',
    );
    const numberOfDependant = cifCode.includes('NK')
      ? result(state, 'form.CCForm5.values.numberOfDependant', '')
    ...
    const currentAddressStatus = result(
      state,
      'form.CCForm6.values.currentAddressStatus',
      '',
    );
    const staySince = result(
      state,
      'form.CCForm6.values.currentAddressSince',
      '',
    );
    const creditTotal = result(state, 'form.CCForm6.values.creditTotal', '');
    const sourceOfFundPayment = result(
      state,
      'form.CCForm6.values.sourceOfFundPayment',
      '',
    );
    ...
    const reasonNoNPWP =
      npwpNumber === ''
        ? result(state, 'form.CCForm7.values.reasonNoNPWP', '')
        : '';

    // Eighth Form
    const workStatus = result(state, 'form.CCForm8.values.workStatus', '');
    const startWork = result(state, 'form.CCForm8.values.startWork', '');
    const startBusiness = result(
      state,
      'form.CCForm8.values.startBusiness',
      '',
    );
    const savingStatement1 = result(
      state,
      'form.CCForm8.values.savingStatement1',
      '',
    );
    const savingStatement2 = result(
      state,
      'form.CCForm8.values.savingStatement2',
      '',
    );
    const savingStatement3 = result(
      state,
      'form.CCForm8.values.savingStatement3',
      '',
    );
    const latestPayslipImage = result(
      state,
      'form.CCForm8.values.latestPayslip',
      '',
    );

    // Ninth Form
    const emergencyFullName = result(
      state,
      'form.CCForm9.values.emergencyFullName',
      '',
    );
    const emergencyRelationship = result(
      state,
      'form.CCForm9.values.emergencyRelationship',
      '',
    );
    const emergencyPhone = result(
      state,
      'form.CCForm9.values.emergencyPhone',
      '',
    );
    const emergencyCountry = isEmpty(result(state, 'form.CCForm9.values', {}))
      ? ''
      : 'Indonesia';
    const emergencyProvince = result(
      state,
      'form.CCForm9.values.emergencyProvince',
      '',
    );
    const emergencyCity = result(
      state,
      'form.CCForm9.values.emergencyCity',
      '',
    );
    const emergencyDistrict = result(
      state,
      'form.CCForm9.values.emergencyDistrict',
      '',
    );
    const emergencySubDistrict = result(
      state,
      'form.CCForm9.values.emergencySubDistrict',
      '',
    );
    const emergencyPostalCode = result(
      state,
      'form.CCForm9.values.emergencyPostalCode',
      '',
    );
    ...
    const emergencyRtRw =
      emergencyRT === '' && emergencyRW === ''
        ? ''
        : emergencyRT + '/' + emergencyRW;
    const emergencyStreetAddress = result(
      state,
      'form.CCForm9.values.emergencyStreetAddress',
      '',
    );

    // Delivery form
    const deliverTo = result(
      state,
      'form.cardDelivery.values.deliveryMode',
      {},
    );
    const deliveryCode = isEmpty(deliverTo)
      ? ''
      : result(deliverTo, 'label', '') === language.CREDITCARD__CURRENT_ADDRESS
      ? '1'
      : result(deliverTo, 'label', '') === language.CREDITCARD__KTP_ADDRESS
      ? '2'
      : '3';

    const checkpointPage = checkpoint
      ? result(state, 'checkpoint.pageName', '')
      : pageName;

    const dataAdditional = {
      customer_id: formid.includes('CCO')
        ? result(state, 'customerIdOrami.customer_id', '')
        : '',
        ...
        if (checkpoint) {
          ...
          dispatch(
            NavigationActions.navigate({
              routeName: pageName,
              params: {name, ticketCode, mobileNumber},
            }),
          );
        } else {
          dispatch(actionCreators.hideSpinner());
...
export function getExistingData(refferalCodeOrami) {
  return (dispatch, getState) => {
    ...
    return api
      .getCcDataCif(payload, dispatch)
      .then(res => {
        dispatch(
          actionCreators.saveCheckpoint(result(res, 'data.data.dataUser')),
        );
        ...
        dispatch(
          change(
            'CCForm2',
            'province',
            openAccountCoreData(result(dataFromCore, '0~6~24', '')),
          ),
        );
        dispatch(
          change(
            'CCForm2',
            'city',
            openAccountCoreData(result(dataFromCore, '0~6~23', '')),
          ),
        );
        dispatch(
          change(
            'CCForm2',
            'district',
            openAccountCoreData(result(dataFromCore, '0~6~21', '')),
          ),
        );
        dispatch(
          change(
            'CCForm2',
            'subDistrict',
            openAccountCoreData(result(dataFromCore, '0~6~22', '')),
          ),
        );
        dispatch(
          change('CCForm2', 'postalCode', result(dataFromCore, '0~6~25', '')),
        );
        ...
        dispatch(actionCreators.hideSpinner());
        dispatch(
          change('CCForm1', 'ktpId2', result(dataDukcapil, '0~2~5', '')),
        );
        dispatch(
          change(
            'CCForm1',
            'maritalStatus2',
            result(dataDukcapil, '0~2~6', ''),
          ),
        );
        ...
        dispatch(
          change(
            'CCForm1',
            'mothersMaiden2',
            result(dataDukcapil, '0~2~8', ''),
          ),
        );
        dispatch(
          change('CCForm1', 'birthPlace', result(dataDukcapil, '0~2~9', '')),
        );

        dispatch(
          change('CCForm2', 'country2', result(dataDukcapil, '0~3~1', '')),
        );
        dispatch(
          change('CCForm2', 'province2', result(dataDukcapil, '0~3~2', '')),
        );
        dispatch(change('CCForm2', 'city2', result(dataDukcapil, '0~3~3', '')));
        dispatch(
          change('CCForm2', 'district2', result(dataDukcapil, '0~3~4', '')),
        );
        dispatch(
          change('CCForm2', 'subDistrict2', result(dataDukcapil, '0~3~5', '')),
        );
        dispatch(
          change('CCForm2', 'postalCode2', result(dataDukcapil, '0~3~6', '')),
        );
        ...
        dispatch(
          change(
            'CCForm2',
            'streetAddress2',
            result(dataDukcapil, '0~3~8', ''),
          ),
        );

        dispatch(
          actionCreators.saveOpenAccountData({
            name,
            email,
            mobileNumber,
            ktpId,
        ...
        dispatch(
          NavigationActions.navigate({
            routeName: 'EmailForm',
            params: {existing: true, refferalCodeOrami},
          }),
        );
      })
      .catch(err => {
        ...
        if (errorCode === '98') {
          ...
          const mobileNumber =
            formatMobileNumberEmoneyRegistration(
              result(state, 'openAccountData.mobileNumber', ''),
            ) === ''
              ? ' '
              : formatMobileNumberEmoneyRegistration(
                  result(state, 'openAccountData.mobileNumber', ''),
          ...
          const username = 'EFORMCENTRAL';
          const password = 's3cuR3p455w0rD3f0rM';
          const isNotEncrypt = result(
            state,
            'config.InactiveEncryptEformKey',
            null,
          );
          let payload = {};
          let apiName = '';
          ...
          dispatch(actionCreators.showSpinner());
          return api[apiName](payload, dispatch).then(() => {
            dispatch(actionCreators.hideSpinner());
            dispatch(
              StackActions.reset({
                index: 1,
      ...
      .then(res => {
        if (result(res, 'data.responseCode', '') === '00') {
          const checkpointData = result(
            res,
            'data.data.dataQuery.0.rawData.dataSubmit',
            {},
          );
          const checkpointPageName = result(
            res,
            'data.data.dataQuery.0.pageName',
            '',
          );
          const checkpointEmail = result(
            res,
            'data.data.dataQuery.0.email',
            '',
          );
          const pageName = changeFormByCode(checkpointPageName, formCode);
          const statusAddress = result(
            res,
            'data.data.dataQuery.0.statusAddress',
            '',
          );
          let useKtp = false;
          ...
          const npwpNumber = result(
            state,
            'form.CCForm7.values.npwpNumber',
            '',
          );
          const imageNpwp = result(
            state,
            'form.npwpImage.values.npwpImage',
            '',
          );
          ...
          dispatch(actionCreators.hideSpinner());
          const npwpImage = result(
            checkpointData,
            'stage2.2.value.0.value',
            '',
          );
          ...
          if (formCode.includes('CCO')) {
            ...
            dispatch(actionCreators.showSpinner());
            return api
              .checkEmailOrami(payload, dispatch)
              .then(res => {
                dispatch(actionCreators.hideSpinner());
                const responseCode = result(res, 'data.responseCode', '');
                const customer_id = result(res, 'data.customer_id', '');
                dispatch(
                  actionCreators.saveCustomerIdOrami({
                    customer_id: customer_id,
                  }),
                );
                if (responseCode === '00') {
                  dispatch(
                    NavigationActions.navigate({routeName: 'Checkpoint'}),
                  );
                } else {
...
export function checkKtpDukcapil(navigation = {}, value = {}) {
  return (dispatch, getState) => {
    ...
    const maritalStatus = result(
      state,
      'form.CCForm1.values.maritalStatus.code',
      '',
    );
    const dob = moment(
      result(state, 'form.CCForm1.values.birthDate', ''),
...
export function checkFromEDW(navigation = {}, value = {}, resData = {}) {
  return (dispatch, getState) => {
    ...
    return api
      .getCustomerProfile(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const responseCode = result(res, 'data.responseCode', '');
        const resCustProfile = result(
          res,
          'data.responseGetCustomerProfileForUpgradeMap',
          {},
        );
            ...
            if (ccCode.includes('CC')) {
              dispatch(
                change(
                  'CCForm1',
                  'ktpId2',
                  result(resData, 'data.dataDukcapil.0~2~5', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm1',
                  'maritalStatus2',
                  result(resData, 'data.dataDukcapil.0~2~6', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm1',
                  'dob2',
                  result(resData, 'data.dataDukcapil.0~2~7', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm1',
                  'mothersMaiden2',
                  result(resData, 'data.dataDukcapil.0~2~8', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm1',
                  'birthPlace',
                  result(resData, 'data.dataDukcapil.0~2~9', ''),
                ),
              );
              ...
              dispatch(
                change(
                  'CCForm2',
                  'country2',
                  result(resData, 'data.dataDukcapil.0~3~1', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm2',
                  'province2',
                  result(resData, 'data.dataDukcapil.0~3~2', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm2',
                  'city2',
                  result(resData, 'data.dataDukcapil.0~3~3', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm2',
                  'district2',
                  result(resData, 'data.dataDukcapil.0~3~4', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm2',
                  'subDistrict2',
                  result(resData, 'data.dataDukcapil.0~3~5', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm2',
                  'postalCode2',
                  result(resData, 'data.dataDukcapil.0~3~6', ''),
                ),
              );
              dispatch(
                change(
                  'CCForm2',
                  'streetAddress2',
                  result(resData, 'data.dataDukcapil.0~3~8', ''),
                ),
              );

              const rtrw2 = split(
                result(resData, 'data.dataDukcapil.0~3~7', ''),
                '/',
                2,
              );
              const rt2 = rtrw2[0] || '';
      ...
      .catch(err => {
        dispatch(actionCreators.hideSpinner());
        Toast.show(
          getErrorMessage(err, language.EMONEY__EDW_PROFILE_FAILED),
          Toast.LONG,
        );
      });
  };
  ...
  return (dispatch, getState) => {
    ...
    return api
      ...
      .then(res => {
        ...
        const customer_id = result(res, 'data.customer_id', '');
        dispatch(
          actionCreators.saveCustomerIdOrami({customer_id: customer_id}),
        );
...
export function verifyOtpEform() {
  return (dispatch, getState) => {
    ...
    const mobileNumber = result(
      state,
      'form.identifyUserForm.values.phone',
      '',
    );
    const email = result(state, 'form.identifyUserForm.values.email', '');
    const referralCode = result(
      state,
      'form.identifyUserForm.values.referralCode',
      '',
    );
    const captchaInput = result(
      state,
        ...
        res => {
          if (!res) {
            PermissionsAndroid.request(
              ...
              if (result === 'granted') {
                return api
                  .verifyOtpEform(payload, dispatch)
                  .then(() => {
                    dispatch(
                      actionCreators.saveOpenAccountData({
                        name,
                  ...
                  .catch(err => {
                    dispatch(actionCreators.hideSpinner());
                    Toast.show(
                      getErrorMessage(err, language.CREATE_ACCOUNT__OTP_ERROR),
                      Toast.LONG,
                    );
                  });
              } else {
...
export function receiveCreditCardCitySecond() {
  return (dispatch, getState) => {
    ...
    const res = result(
      state,
      'form.CCForm9.values.emergencyProvince.value',
      '',
    );
...
export function prefilledFromDukcapil(navigation) {
  return (dispatch, getState) => {
    ...
    if (usingKtpData === true) {
      dispatch(
        change('CCForm2', 'province', {
          code: result(listDukcapil, '0~3~2', ''),
          name: result(listDukcapil, '0~3~2', ''),
          value: result(listDukcapil, '0~3~2', ''),
        }),
      );
      dispatch(
        change('CCForm2', 'city', {
          code: result(listDukcapil, '0~3~3', ''),
          name: result(listDukcapil, '0~3~3', ''),
          value: result(listDukcapil, '0~3~3', ''),
        }),
      );
      dispatch(
        change('CCForm2', 'district', {
          code: result(listDukcapil, '0~3~4', ''),
          name: result(listDukcapil, '0~3~4', ''),
          value: result(listDukcapil, '0~3~4', ''),
        }),
      );
      dispatch(
        change('CCForm2', 'subDistrict', {
          code: result(listDukcapil, '0~3~5', ''),
          name: result(listDukcapil, '0~3~5', ''),
          value: result(listDukcapil, '0~3~5', ''),
          zipCode: result(listDukcapil, '0~3~6', ''),
        }),
      );
      const postalCode = result(listDukcapil, '0~3~6', '');
      if (postalCode === '') {
        const inputtedPostalCode = result(
          state,
          'form.PostalCodeForm.values.postalCode',
          '',
        );
      ...
      dispatch(
        change('CCForm2', 'streetAddress', result(listDukcapil, '0~3~8', '')),
      );
    }
...
export function openCreditCard(refferalCodeOrami, nav) {
  return (dispatch, getState) => {
    ...
    const isLogin =
      !isEmpty(result(state, 'user', {})) &&
      (result(nav, 'index', 0) > 0 || loginRoute === 'Main');
    const accountList = result(state, 'accounts', []);
    const cifCode = result(state, 'user.profile.customer.cifCode', '');
    const emoneyKycOnly =
      !find(accountList, {accountType: 'SavingAccount'}) &&
    ...
    if (isLogin) {
      if (!startsWith(cifCode, 'NK')) {
        if (emoneyKycOnly) {
          dispatch(
            NavigationActions.navigate({routeName: 'CreditCardKTPCamera'}),
          );
        } else {
      ...
      dispatch(
        NavigationActions.navigate({
          routeName: 'CreateCCAccount',
          params: {refferalCodeOrami},
        }),
      );
    }
  };
...
export function generatePhotoKTP(base64) {
  return (dispatch, getState) => {
    ...
    const requestData = {
      mobileNumber,
      photoBase64: result(base64, 'base64', ''),
      namePhoto: '0~1~1',
      lang,
    };
  ...
  return (dispatch, getState) => {
    ...
    if (formid === 'CCO-SIMOBI-002') {
      if (customer_id !== null) {
        ...
        const gender =
          result(state, 'listDukcapil.0~2~11', '') === 'Laki-Laki' ? 'M' : 'F';
        ...
        const dob = isValidKyc
          ? moment(result(state, 'listDukcapil.0~2~7', '')).format('DD/MM/YYYY')
          : moment(result(state, 'form.CCForm1.values.birthDate', '')).format(
              'DD/MM/YYYY',
            );
        ...
        return api
          .registerVerifyOrami(payload, dispatch)
          .then(res => {
            const entity_id = result(res, 'data.data.entity_id', '');
            dispatch(
              actionCreators.saveCustomerIdOrami({customer_id: entity_id}),
            );
            dispatch(createCreditCardForm(statusForm, pageName, checkpoint));
            dispatch(actionCreators.hideSpinner());
          })
...
export function requestOtpEform() {
  return (dispatch, getState) => {
    ...
    return api
      .requestOtpEform(payload, dispatch)
      .then(res => {
...
export function resendOtpEform() {
  return (dispatch, getState) => {
    ...
    return api.requestOtpEform(payload, dispatch).catch(err => {
      Toast.show(
        getErrorMessage(
          err,
          language.ERROR_MESSAGE__COULD_NOT_SEND_SMS_VERIFICATION,
...
export function accountRegistered() {
  return dispatch => {
    ...
    const goToRegisterAtm = () => {
      Promise.all([dispatch(logout())]).then(() => {
