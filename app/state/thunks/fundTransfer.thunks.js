...
import {
  ...
  getDayName,
  getTransactionType,
  getTransferPossibleAccountsBIFast,
  getMaskingAccount,
...
import {
  updateInvoiceSplitBillTF,
  reminderMaxBalanceSplitBill,
} from '../../state/thunks/splitBill.thunks';
import md5 from 'md5';
...
export function getPayeeName() {
  return (dispatch, getState) => {
    const {
      ...
      payeeAccNo = '',
      payeeType = {},
    } = result(getState(), 'form.addPayee.values', {});
    const bankId =
      result(bank, 'id', '') === ''
        ? result(bank, 'bankId', '')
        : result(bank, 'id', '');
    const accNo =
      payeeType.value !== 'emoney' || payeeAccNo.substring(0, 4) === '3808'
        ? payeeAccNo
        : result(bank, 'companyCode', '08') === '08'
        ? result(bank, 'prefixEmoney', '38') + payeeAccNo
        : result(bank, 'prefixEmoney', '38') +
          result(bank, 'companyCode', '08') +
          payeeAccNo;
    ...
    if (!isEmpty(bank) && payeeAccNo !== '') {
      ...
      if (
        accNo.length > 0 &&
        (bank.isSinarmas ||
          bank.networkEnabled === 'yes' ||
          payeeType.value === 'emoney')
      ) {
        ...
        return api
          ...
          .then(payeeDetails => {
            ...
            dispatch(
              change('addPayee', 'originalName', payeeDetails.data.targetName),
            );
            dispatch(
              change('addPayee', 'payeeName', payeeDetails.data.targetName),
            );
...
export function getEmoneyPayeeName(payeeAccNo) {
  return (dispatch, getState) => {
    ...
    const bankList = result(
      state,
      'configEmoney.emoneyConfig.listTransferConfig',
      [],
    );
    const bank = find(bankList, bank => bank.companyCode === '08');
    const accNo =
      payeeAccNo.substring(0, 4) === '3808' ? payeeAccNo : '38' + payeeAccNo;
    const isLogin = !isEmpty(result(getState(), 'user', {}));
    const transferMethodType = isLogin ? '' : '1';
    let additional = [];
    if (isLogin) {
          ...
          .then(payeeDetails => {
            ...
            dispatch(
              NavigationActions.navigate({
                ...
                params: {
                  payeeAccNo,
                  emoneyPayeeData: {...payeeDetails.data, bank},
                },
              }),
            );
          })
...
export function confirmTransfer(
  ...
  return (dispatch, getState) => {
    ...
    const currencyObjectAdditional = result(
      currencyRate,
      'currencyObjectAdditional[0]',
      {},
    );
    const sellRateSpread = result(currencyObject, 'spreadSellRate', 0);
    const sellRateSpreadAdditional = result(
      currencyObjectAdditional,
      'spreadSellRate',
      0,
    );
    const transferTime =
    ...
    const isLogin = !isEmpty(result(getState(), 'user', {}));
    const transferMethodType = isLogin ? '' : '1';
    const accountList = getTransferPossibleAccountsBIFast(
      result(getState(), 'accounts', []),
      'ft',
    );
    const confirmTransferPayload = isBiFast
      ? middlewareUtils.prepareConfirmTransferBiFastPayload(
          formValues,
          payee,
          transType,
          time,
    ...
    const CONFIRMTRANSFER =
      transferMode === 'bifast' || isBiFast
        ? api.confirmTransferBiFast
        : api.confirmTransfer;
    ...
    if (currencyDeterminant === currencyTarget) {
      if (currencyTarget === 'IDR') {
        ...
        if (sellRateSpread === null || sellRateSpread === 0) {
          amountValastoIDR =
            result(formValues, 'amount') * sellRateSpreadAdditional;
        } else {
    ...
    return CONFIRMTRANSFER(confirmTransferPayload, additional, dispatch)
      ...
      .catch(err => {
        ...
        const responseMessageHeader = result(
          err,
...
export function transfer(
  ...
  return (dispatch, getState) => {
    ...
    const serverTimeNew = String(
      moment(showTime).add(gapTime, 'seconds').format('HH:mm'),
    );
    const sendDate =
      (moment(serverTimeNew, 'HH:mm').isBefore(
      ...
      (moment(serverTimeNew, 'HH:mm').isBefore(
        ...
        cutOffMsg === null) ||
      (moment(serverTimeNew, 'HH:mm').isAfter(
        moment(result(timeConfig, 'startTimeSkn_CUTOFF'), 'HH:mm'),
      ) &&
        cutOffMsg === null) ||
      (moment(serverTimeNew, 'HH:mm').isAfter(
        ...
        cutOffMsg === null)
        ? timeConfig.coreBusinessDateV3
        : timeConfig.coreNextBusinessSknDateV3 ||
          timeConfig.coreNextBusinessRtgsDateV3;
    if (transferType === 'skn' || transferType === 'rtgs') {
      ...
      time = getTransferTime(
        moment(result(timeConfig, 'cutOffTimeSkn'), 'HH:mm'),
    ...
    if (
      ...
      (time === 'today' && transferType === 'skn') ||
      (time === 'today' && transferType === 'rtgs')
    ) {
      isFundTransfer = 'yes';
    } else if (
      (time === 'nextWorking' && transferType === 'skn') ||
      (time === 'nextWorking' && transferType === 'rtgs') ||
      transferType === 'bifast' ||
      isBiFast
    ) {
    ...
    let currency = '';
    if (isValas) {
      const currencyDeterminant = String(
        result(
          transferFormData,
          'currency.name',
          String(result(sourceAccount, 'currency', '')),
        ),
      );
      const currencyTarget = isValas
        ? currencyDeterminant
        : String(result(sourceAccount, 'currency', ''));
    ...
    if (isSplitBill) {
      ...
      modalOptions = {
        ...
        accountFrom:
          type === 'cardlessWithdrawal'
            ? sourceAccount
            : getMaskingAccount(sourceAccount),
        currencyValas: isValas ? currency : currency,
        currencyRate,
        resData,
        isFundTransfer,
    ...
    const allTransfers = result(
      storeState,
      'lastFundTransactions.allTransactions',
      [],
    );
    const allTransactions = result(
      storeState,
      'lastCreditCardTransactions.allTransactions',
      [],
    );
    let trxType;
    ...
    const isNewAccount =
      result(payeeAccount, 'id', '') === '' ||
      result(payeeAccount, 'id', '') === null;
    dispatch(
    ...
    const transferTypeRemittance = result(
      resData,
      'transferTransaction.transactionType',
      '',
    );
    const transferMode = result(transferFormData, 'transferMode', '');
    const transferPayload = isSplitBill
      ? middlewareUtils.prepareTransferPayload(
          data,
          payeeAccount,
          transferReferenceNumberSplitbill,
          easyPin,
          smsOtp,
          simasToken,
          type,
      ...
      ? middlewareUtils.prepareRemittancePayload(
          data,
          payeeAccount,
      ...
      : isBiFast
      ? middlewareUtils.prepareTransferBiFastPayload(
          data,
          payeeAccount,
          transferReferenceNumber,
          easyPin,
    ...
    const TRANSFER =
      transferTypeRemittance === 'remmitance'
        ? api.transferRemittance
        : transferMode === 'bifast' || isBiFast
        ? api.transferBiFast
        : api.transfer;
    return TRANSFER(transferPayload, dispatch)
      .then(res => {
        ...
        const ipassportAdditional = result(
          state,
          'additionalApiPayload.ipassport',
          '',
        );
        const ipassport =
          ipassportUser === '' || ipassportUser === null
            ? ipassportAdditional
            : ipassportUser;
        ...
        const uniCode =
          md5(transactionCode) === result(res, 'data.checkingCode', '');
        ...
        if (responseCode === '00' && uniCode) {
          ...
          if (isBfr === true) {
            ...
            dispatch(
              NavigationActions.navigate({
                ...
                params: {isValas, resDataTrf, isBillPay},
              }),
            );
          } else if (isSplitBill) {
            dispatch(
          ...
          isSplitBill
            ? dispatch(updateInvoiceSplitBillTF(dataUpdateInvoiceSB))
            : null;
          ...
          if (type === 'creditCard') {
            ...
            dispatch(actionCreators.clearBillerDescFav());
            dispatch(
              actionCreators.updateLastCreditCardPayment(
                middlewareUtils.getCreditCardTransferHistory(id, payeeAccount),
              ),
            );
            ...
            set(
              storageKeys.ALL_CREDIT_CARD_TRANSACTIONS,
              allTransactions,
            ).catch(err => {
              Toast.show(
                getErrorMessage(
            ...
            set(storageKeys.ALL_PAYEES, allTransfers).catch(err => {
              Toast.show(
                getErrorMessage(
                  err,
                  language.ERROR_MESSAGE__COULD_NOT_STORE_RECENT_PAYEE_DETAILS,
                ),
                Toast.LONG,
              );
            });
          ...
          if (easyPinAttempt === 'invalid') {
            ...
            if (res.AddPayeeFailed) {
              modalOptions = {...modalOptions, transactionType: 'Add Payee'};
            } else if (type === 'creditCard') {
      ...
      return api
        ...
        .catch(err => {
          Toast.show(
            getErrorMessage(
              err,
              language.ERROR_MESSAGE__COULD_NOT_DELETE_PAYEE,
            ),
            Toast.LONG,
          );
        });
...
export function setupFundTransfer(
  payee,
  amount,
  isSplitBill,
  dataTransRefNum,
    ...
    return dispatch(populateConfigData())
      ...
      .catch(err => {
        ...
        const errorMessage = getErrorMessage(
          err,
          language.ERROR_MESSAGE__COULD_NOT_SETUP_TRANSFER,
        );
  ...
  return dispatch => {
    ...
    return dispatch(populateConfigData())
      ...
      .catch(err => {
        ...
        const errorMessage = getErrorMessage(
          err,
          language.CARDLESSWITHDRAWAL__COULD_NOT_SETUP,
        );
        Toast.show(errorMessage, Toast.LONG);
      });
  };
...
export function inquiryRecurringTransfer() {
  return (dispatch, getState) => {
    ...
    return api
      .getRecurringTransferHistory(payload, dispatch)
      .then(res => {
        dispatch(actionCreators.hideSpinner());
        const data = result(res, 'data.data.dataSFT', []);
        dispatch(actionCreators.updateRecurringData(data));
        dispatch(
          NavigationActions.navigate({routeName: 'RecurringDetailList'}),
        );
      })
      .catch(err => {
        dispatch(actionCreators.hideSpinner());
...
export function editRecurringTransfer(data) {
  return (dispatch, getState) => {
    ...
    const amount = parseInt(
      result(state, 'form.RecurringEditForm.values.amountRecurring', ''),
    );
    dispatch(actionCreators.showSpinner());
    const payload = {
      accessFrom,
    ...
    return api
      .posteditingRecurringTransferHistory(payload, dispatch)
      .then(() => {
        dispatch(actionCreators.hideSpinner());
...
export function deleteRecurringTransfer(data) {
  return (dispatch, getState) => {
    ...
    return api
      .deleteRecurringTransferHistory(payload, dispatch)
      .then(() => {
        dispatch(actionCreators.hideSpinner());
        dispatch(
          StackActions.reset({
...
export function getPayeeNameFavorite(bank) {
  return (dispatch, getState) => {
    const payeeType = result(
      getState(),
      'form.FavoriteBillerForm.values.payeeType',
      {},
    );
    const payeeAccNo = result(
      getState(),
...
export function silentLoginBillpay() {
  return dispatch => {
    dispatch(login({}, true, false, false, false, false, true, 1, 1))
      .then(() => {})
      .catch(err => {
...
export function setupSwiftCodeBankInformation(swiftBank, payee) {
  return (dispatch, getState) => {
    ...
    dispatch(actionCreators.showSpinner());
    return api
      .validateSwiftCode(payload, dispatch)
      .then(res => {
        const responseCode = result(res, 'data.responseCode', '');
...
export function checkTimeOperationRemittance() {
  return (dispatch, getState) => {
    ...
    return api
      .checkOperationTime(payload, dispatch)
      .then(res => {
        const responseCode = result(res, 'data.responseCode', '');
...
export function getCurrencyPurpose() {
  return (dispatch, getState) => {
    ...
    dispatch(actionCreators.showSpinner());
    return api
      .currencyPurpose(payload, dispatch)
      .then(res => {
...
export function getSenderRemittance() {
  return (dispatch, getState) => {
    ...
    dispatch(actionCreators.showSpinner());
    return api
      .detailSenderRemittance(payload, dispatch)
      .then(res => {
...
export function confirmTransferRemittance(
  formValues,
  payee,
  transType,
  getCurrency,
  ...
  return (dispatch, getState) => {
    ...
    const confirmTransferPayload =
      middlewareUtils.prepareConfirmRemittancePayload(
        formValues,
        payee,
        transType,
        time,
    ...
    dispatch(actionCreators.showSpinner());
    return api
      .confirmRemittance(confirmTransferPayload, additional, dispatch)
      .then(res => {
        const uniqueCode = result(res, 'data.uniqueCode', '');
        dispatch(actionCreators.saveUniqeCode(uniqueCode));
        const resData = result(res, 'data', {});
...
export function deleteSelectedPayeeRemittance(payee) {
  return dispatch => {
    ...
    const goDelete = () => {
      ...
      dispatch(actionCreators.hideSinarmasAlert());
      return api
        .deleteFromPayeeListRemittance(payload, dispatch)
        .then(() => {
...
export function setupFundTransferSetLimit(
  payeeNumber,
  payeeName,
  bank,
  currency,
...
export function popUpSetLimitShortCut(
  payeeNumber,
  payeeName,
  bank,
  currency,
...
export function setupFromSetLimit(
  name,
  accountNumber,
  accountType,
  bank,
...
export function easyPinDataToAddLimitTransactionFund(
  payeeName,
  payeeNumber,
  bank,
  currency,
  ...
  return (dispatch, getState) => {
    ...
    const debitAccountNumber =
      result(state, 'form.fundTransfer.values.myAccount.accountNumber', '') ||
      result(state, 'defaultAccount.accountNumber', '');
    const debitAccountName =
      result(state, 'form.fundTransfer.values.myAccount.name', '') ||
    ...
    const limitPerDay = result(
      state,
      'form.autoFilledSetLimitTransferFund.values.limitPerDay',
      '',
    );
    const limitPerTransaction = result(
      state,
      'form.autoFilledSetLimitTransferFund.values.limitPerTransaction',
      '',
    );
    ...
    dispatch(
      NavigationActions.navigate({routeName: 'EasyPinSetLimitEditFund'}),
    );
    dispatch(actionCreators.hideSpinner());
  };
}

export function easyPinDataToAddLimitTransactionConfirm(
  payeeName,
  payeeNumber,
  payeeTargetAccNo,
  payeeTargetAccName,
  ...
  return (dispatch, getState) => {
    ...
    dispatch(actionCreators.saveAddLimitTransaction({payload}));
    dispatch(
      NavigationActions.navigate({routeName: 'EasyPinSetLimitEditFundConfirm'}),
    );
    dispatch(actionCreators.hideSpinner());
  };
}

export function setupFundTransferSetLimitVerify(
  payeeName,
  payeeNumber,
  bank,
  currency,
  ...
  return dispatch => {
    ...
    return dispatch(populateConfigData())
      .then(() => {
        ...
        dispatch(
          easyPinDataToAddLimitTransactionFund(
            payeeName,
            payeeNumber,
            bank,
            currency,
...
export function setupFundTransferSetLimitVerifyConfirm(
  payeeName,
  payeeNumber,
  payeeTargetAccNo,
  payeeTargetAccName,
  ...
  return dispatch => {
    ...
    return dispatch(populateConfigData())
      .then(() => {
        ...
        dispatch(
          easyPinDataToAddLimitTransactionConfirm(
            payeeName,
            payeeNumber,
            payeeTargetAccNo,
            payeeTargetAccName,
...
export function confirmTransferSetLimit(formValues, payee, isKyc) {
  return (dispatch, getState) => {
    ...
    const transferMethodType = isLogin ? '' : '1';
    const confirmTransferPayload =
      middlewareUtils.prepareConfirmTransferPayloadSetLimit(
        formValues,
        payee,
        time,
        transferMethodType,
...
export function transferSetLimit(
  transferFormData,
  payee,
  myAccount,
  type,
  ...
  return (dispatch, getState) => {
    ...
    const detailsShow =
      type === 'cardlessWithdrawal'
        ? phoneNumber
        : result(payee, 'payeeAccount.accNo', '') || accountNumber;
    let modalOptions = {
    ...
    const initiatedTime =
      transferTime === ''
        ...
        : dayRecurr +
          ', ' +
          moment(transferFormData.transferTime).format('DD MMM YYYY');
    const dataDetail = middlewareUtils.prepareDataDetailTransferSetLimit(
      transferFormData,
      payee,
      myAccount,
      type,
    ...
    const iscounterTrxData = result(storeState, 'counterTrxData', true);
    const isNewAccount =
      result(payee, 'payeeAccount.id', '') === '' ||
      result(payee, 'payeeAccount.id', '') === null;
    dispatch(
      actionCreators.showPaymentModal({
        ...modalOptions,
        transactionType: trxType,
    ...
    const transferPayload = middlewareUtils.prepareTransferPayloadSetLimit(
      data,
      payee,
      transferReferenceNumber,
      easyPin,
    ...
    return api
      ...
      .then(res => {
        ...
        if (type === 'creditCard') {
          ...
          dispatch(
            actionCreators.updateLastCreditCardPayment(
              middlewareUtils.getCreditCardTransferHistory(id, payee),
            ),
          );
          allTransactions.push({
          ...
          set(storageKeys.ALL_CREDIT_CARD_TRANSACTIONS, allTransactions).catch(
            err => {
              Toast.show(
                getErrorMessage(
                  err,
          ...
          dispatch(
            actionCreators.updateLastTransactions(
              middlewareUtils.getFundTransferHistory(id, payee),
            ),
          );
          allTransfers.push({
...
export function goToEditSetLimitFundTransfer(
  payeeName,
  payeeNumber,
  bank,
  currency,
      ...
      .then(() => {
        ...
        dispatch(
          StackActions.reset({
            ...
            actions: [
              NavigationActions.navigate({
                routeName: 'FundTransferPaymentFromSetLimit',
                params: {
                  payeeName,
                  payeeNumber,
                  bank,
...
export function confirmTransferFromSetLimit(
  formValues,
  payee,
  transType,
  getCurrency,
...
export function getPayeeNameBiFast(biFastProxy, isBiFast) {
  return (dispatch, getState) => {
    const {bank = {}, payeeAccNo = ''} = result(
      getState(),
      'form.addPayee.values',
      {},
    );
    const accountList = getTransferPossibleAccountsBIFast(
      result(getState(), 'accounts', []),
    ...
    if (isEmpty(bank) && payeeAccNo === '') {
      ...
      dispatch(actionCreators.showSpinner());
      return api
        .getPayeeNameBiFast(payload, additional, dispatch)
        .then(payeeDetails => {
          const payeeStatus = result(payeeDetails, 'data', {});
          ...
          dispatch(
            NavigationActions.navigate({
              routeName: 'AddPayee',
              params: {biFastProxy, isBiFast, proxyType, accNo},
            }),
          );
        })
        .catch(err => {
...
export function deleteSelectedPayeeBiFast(payee) {
  return dispatch => {
    ...
    const goDelete = () => {
      ...
      dispatch(actionCreators.hideSinarmasAlert());
      return api
        .deleteFromPayeeListBiFast(payload, dispatch)
        .then(() => {
