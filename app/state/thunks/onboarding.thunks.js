  ...
  return (dispatch, getState) => {
    ...
    return loginAPI({username: normalisePhoneNumber(username), password, easyPin, smsPriority, deviceInfoLogin, versionScope}, isLockedDevice, dispatch).then((res) => {
      ...
      if (errorOBM === 'true') {
        ...
        if (status === 429) {
          Toast.show(getErrorMessage(err, language.COMMON__TIMEOUT_TITLE1_F5), Toast.LONG);
        } else {
          Toast.show(getErrorMessage(err, language.ERROR_MESSAGE__COULD_NOT_LOG_IN), Toast.LONG);
        }
      }
  ...
  return (dispatch, getState) => {
    ...
    return dispatch(populateConfigData()).
      ...
      then(({data, config}) => {
        ...
        const params = {TXID: payload.TXID, panNumber, newUserMobile: result(data, 'attributeMap.mobileNumberMasking', false), isForgetPassword, goToPin};
        dispatch(resetToLandingAndNavigate('OTP', params));
      }).catch((err) => {
        dispatch(actionCreators.hideSpinner());
        const errorCode = result(err, 'data.responseCode', '');
...
export function prepareCompletedOnboarding (maskedUsername, forgetEasyPin = false, isRegistDeeplink = false, isPinCreation = false) {
...
export function setEasyPin (easyPinData, maskedUsername, isResetEasypin = false, isRegistDeeplink = false, firebaseEmoney = false, isPinCreation = false) {
    ...
    return api.easyPinRegister(middlewareUtils.prepareEasyPinRegister(easyPinConfirm, isResetEasypin, token, ipassport, isFromEmoney, transRefNum, ipassportEmoney), dispatch).
      then(() => {
        ...
        if (isFaceRegistered || skipped || isRegistDeeplink) {
          ...
          dispatch(prepareCompletedOnboarding(maskedUsername, isResetEasypin, isRegistDeeplink, isPinCreation));
          dispatch(destroy('easyPinCreationForm'));
          dispatch(destroy('easyPinCreationConfirmForm'));
        } else {
          const skipFunction = () => {
...
export function activationRegistration (valueForm, registTemporary, firebaseEmoney, typeActivation) {
  return (dispatch, getState) => {
    ...
    const payload = middlewareUtils.prepareActivation(loginName, password, registTemp, utm);
    let adjustEvent;
