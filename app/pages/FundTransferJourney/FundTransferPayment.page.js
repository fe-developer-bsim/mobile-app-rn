...
class FundTransferPaymentPage extends Component {
    ...
    if (this.state.scheduled) {
      ...
      if (getPayeeType(payee, isNetwork) === 'internal' || toEmoney || isNetwork || isValas || isBiFast) {
        ...
        if ((isEmpty(formValues.currency)) || (formValues.currency.name === 'IDR' || formValues.currency.name !== 'IDR')) {
          goToSummary(payee, formValues, getCurrency, currencyRate, {}, isValas, isKyc, isBiFast, isSplitBill, dataTransRefNum, invoiceNumber);
        }
      } else {
        if (isLogin) {
  ...
  selectSourceAcc = () => {
    ...
    let isValas = isSourceIDR || isPayeeIDR;
    const isBiFast = result(navigation, 'state.params.payee.isBiFast', false);
    getSourceAcc(payee, isTopup, isValas, isBiFast, isSplitBill);
...
const mapDispatchToProps = (dispatch) => ({
  ...
  goToSummary: (payee, formValues, getCurrency, currencyRate, newformValues, isValas, isKyc, isBiFast, isSplitBill, dataTransRefNum, invoiceNumber) => {
    dispatch(confirmTransfer(formValues, payee, 'fundTransfer', getCurrency, currencyRate, newformValues, isValas, isKyc, isBiFast, isSplitBill, dataTransRefNum, invoiceNumber));
  },
  getSourceAcc: (payee, isTopup, isValas, isBiFast, isSplitBill) => {
    if (isSplitBill) {
      dispatch(NavigationActions.navigate({
      ...
      dispatch(NavigationActions.navigate({
        ...
        params: {formName: 'fundTransfer', fieldName: 'myAccount', sourceType: 'fundTransfer', payee: payee, isTopup: isTopup, isValas: isValas, isBiFast: isBiFast}
      }));
    }
  },
