...
import AccountSettingsForm, {
  fields,
} from '../../components/OnboardingJourney/UserPasswordSettings.component';
import {
  setLoginAccount as setLoginAccountThunk,
  forgetEasyPin,
  setLoginAccountMigrate,
...
const formConfig = {
  ...
  onSubmit: (values, dispatch, props) => {
    ...
    const isResetEasypin = result(
      props,
      'navigation.state.params.isForgetPassword',
      false,
    );
      ...
      return new Promise((resolve, reject) => {
        ...
        if (blurredField === USERNAME) {
          if (!values[USERNAME]) {
            reject({
              ['_error']:
                language.ONBOARDING__ACCOUNT_VALIDATION_USERNAME_REQUIRED,
            });
      ...
      return new Promise((resolve, reject) => {
        ...
        if (blurredField === USERNAME) {
          if (values[USERNAME].length >= 1) {
            return verifyUsernameAPI(values[USERNAME], dispatch)
              .then(() => {
                resolve(null);
              })
              .catch(() => {
                reject({
                  [USERNAME]:
                    language.ONBOARDING__ACCOUNT_VALIDATION_USERNAME_DUPLICATE,
                });
              });
          } else {
            reject({
...
const mapStateToProps = state => {
  ...
  const toggleIB = result(
    state,
    'form.loginAccountForm.values.isAllowedIB',
    '',
  );
  return {
    currentLanguage,
...
const mapDispatchToProps = dispatch => ({
  setUsername: maskedUsername =>
    dispatch(change('loginAccountForm', 'username', maskedUsername)),
  goToForgotPassword: () => {
    dispatch(
      StackActions.reset({
        index: 0,
...
class AccountSettings extends Component {
  ...
  validationInput =
    ...
    (inputProps = {}, val = '') => {
      ...
      } else if (typeField === 'confirmPassword') {
        if (
          isEmpty(
            isValidPassword(
              val,
  ...
  render() {
    ...
    return (
      <RegisterForm
        ...
        isSecureTextEntryConfirmNewPassword={
          this.state.isSecureTextEntryConfirmNewPassword
        }
