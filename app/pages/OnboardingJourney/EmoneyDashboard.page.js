...
import {
  updateTransactionEmoneyHistory,
  exportTransactionHistory,
} from '../../state/thunks/transactionHistory.thunks';
import toLower from 'lodash/toLower';
import {updateTransactionFiltersEmoney} from '../../state/actions/index.actions';
// import firebase from 'react-native-firebase';
// let Analytics = firebase.analytics();

const DEFAULT_TRANSACTION_FILTER = {
...
const mapStateToProps = state => ({
  ...
  isUndian:
    upperCase(result(state, 'config.flag.flagUndian', 'INACTIVE')) ===
    upperCase('ACTIVE'),
...
const mapDispatchToProps = dispatch => ({
  ...
  goToSwitchpage: () => {
    dispatch(
      NavigationActions.navigate({routeName: 'SetDefaultAutoDebitScreen'}),
    );
  },
});
  ...
  render() {
    ...
    return (
      <AccountMenu
        ...
        goToAllTransactions={this._goToEmoneyHistoryNavigate}
        emoneyAccounts={emoneyAccounts}
        emoney={emoney}
        goToTopUp={goToTopUp}
        gotoTopUpNkc={gotoTopUpNkc}
        cif={cif}
        upgradeKyc={upgradeKyc}
        sendMail={this.sendMail}
        primaryToogleAccount={primaryToogleAccount}
        goToSwitchpage={goToSwitchpage}
        switchAccountToogleBE={switchAccountToogleBE}
      />
    );
  }
}
