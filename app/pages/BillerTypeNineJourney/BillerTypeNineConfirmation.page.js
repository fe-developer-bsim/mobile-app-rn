...
import BillerTypeNineConfirmation from '../../components/BillerTypeNineJourney/BillerTypeNineConfirmation.component';
import {payGenericBillTypeNine, confirmGenericBillTypeNine, triggerAuthBillpay, checkValidityCouponLogin, registerAutoDebit} from '../../state/thunks/genericBill.thunks';
...
const formConfig = {
  form: 'BillerTypeNineConfirmationForm',
  onSubmit: (values, dispatch, {denomination = {}, navigation, payBill, currentAmount, confirmData, isLogin, isSyariah, couponCheck, autoDebitDate}) => {
    const amount = result(denomination, 'id', 0);
    const params = {onSubmit: payBill, amount, isOtp: false};
    const resDataTemp = result(navigation, 'state.params.resData', {});
    const confirmFunction = () => {
      dispatch(confirmGenericBillTypeNine({...confirmData, resDataTemp, autoDebitDate}, true)).then(() => {
...
const ConnectedForm = reduxForm(formConfig)(BillerTypeNineConfirmation);

class BillerTypeNineConfirmationPage extends React.Component {
  static propTypes = {
  ...
  payBill = () => {
    const {payGenericBillTypeNine, isLogin, navigation, isUseSimas, isAutoDebit, setAsAutodebit, autoDebitDate} = this.props;
    ...
    if (registerAutodebitOnly) {
      ...
      payGenericBillTypeNine({nextAutodebit, ...data}, isBillPay);
    }
  }
  componentWillMount () {
  ...
  render () {
    ...
    const {triggerAuth, removeCoupon, timeEndCoupon, gapTimeServer, navigation,
      timeStartCoupon, dateEndCoupon, dateStartCoupon, usingFromLine, state,
      minAmount, maxAmount, showFavorite, favoriteBill, removeFavorite,
      billerFavorite, currency, isLogin, couponCheck, isAutoDebit, autoDebitDate} = this.props;
    ...
    const isSyariah = checkShariaAccount(result(navParams, 'accountNo', {})) && currency !== 'simaspoin';
    const isADebit = result(navParams, 'isADebit', false);
    return <ConnectedForm {...navParams} goToCoupon={this.goToCoupon} timeEndCoupon={timeEndCoupon} gapTimeServer={gapTimeServer} removeCoupon={removeCoupon} navigation={navigation}
      ...
      couponUse={this.state.voucherDescription} triggerAuth={triggerAuth} payBill={this.payBill}  showFavorite={showFavorite} favoriteBill={favoriteBill} removeFavorite={removeFavorite}
      billerFavorite={billerFavorite} isSyariah={isSyariah} isLogin={isLogin} isUseSimas={isUseSimas}
      state={state} couponCheck={couponCheck} isAutoDebit={isAutoDebit} isADebit={isADebit} autoDebitDate={autoDebitDate}/>;
...
const mapDispatchToProps = (dispatch) => ({
  ...
  payGenericBillTypeNine: (data, isBillPay) => dispatch(payGenericBillTypeNine(data, isBillPay)),
...
export default connect(mapStateToProps, mapDispatchToProps)(BillerTypeNineConfirmationPage);
