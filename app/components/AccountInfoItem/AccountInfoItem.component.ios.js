...
class AccountInfoItem extends React.Component {
  ...
  closeSimasTara = (accountNumber, product, sourceAccNoSimasTara) => () => {
    const {goToCloseSimasTara} = this.props;
    this.setState({showDetail: false});
    goToCloseSimasTara(accountNumber, product, sourceAccNoSimasTara);
  }
  ...
  showDetail = () => () => {
    const {detailLockedAmount, goToSimasTaraDetail, accountInfo} = this.props;
    const accountNumber = result(accountInfo, 'accountNumber', '');
  ...
  render () {
    const {accountInfo = {}, index, details, onItemPress = noop, setVisibility, accountVisibility, activeTab, getMmq, getMmqDataDetail, goToBillerVirtualAccount, loadBalances, currentCarouselIndex, goToSimasTaraDetail, approveAplication, isUsingDigisign, accountsCust, detailLocked, goToshowDormantModal} = this.props;
    const {accountType, NamaProgram, cardStatus, cardBase, accountStatus, balances = {}} = accountInfo;
    ...
    return (
      <View style={styles.container}>
            ...
            <Modal
              ...
              <View style={styles.centeredView}>
                <View style={styles.modalView}>
                  <Touchable style={styles.simasTaraDetailArrowClose} onPress={this.closeDetail()}>
        ...
        {typeOfLoan === 'PGO' ?
          <View>
            <ElevatedView elevation={2} style={styles.imageContainer}>
                    ...
                    <View style={styles.rowLoan}>
                      <View style={styles.blockOne}>
                        ...
                        {status === 'AUDIT_SUCCESS' ?
                          <View style={styles.circleCheck}>
                            <SimasIcon name={'check-black'} size={9} style={styles.checkList}/>
                          </View> : status === 'AUDIT_REFUSE' ? <View style={styles.circleCheck}>
                            <SimasIcon name={'fail'} size={7} style={styles.checkListCross}/>
                          </View>
                            : <View style={styles.circleCheck}/>
                        }
            ...
            <View>
              <View>
                <AccountDetails index={index} accountInfo={accountInfo} {...details} goToSimasTaraDetail={goToSimasTaraDetail}/>
              </View>
              {checkStatusDetail === '1' &&
                    ...
                    {isUsingDigisign ?
                      ...
                      statusOpeningAccount !== '' ?
                        <View style={styles.detailRowSA}>
                          ...
                          <View>
                            <Text style={styles.textTitleLoanOAIOSSA}>
                              {language.PGO__DISPLAY_CARD_APLICATION_RECEIVED}
                              {language.GENERAL__COMMA}
                              {aplicationReceived !== '' ? moment.unix(aplicationReceived / 1000).format('DD MMM YYYY [at] HH:mm') : ''}
                            </Text>
              ...
              : openingAccount === 'openingLOAN' ?
                <View>
                  <ElevatedView elevation={2} style={styles.imageContainer}>
                    <ImageBackground source={itemTheme.image} borderRadius={15} style={styles.backgroundImage}>
                      <View style={styles.detailContainer}>
                        ...
                        <View style={styles.boxSubtitleIOS}>
                          <Text style={styles.textSubtitleLoan}>{language.DASHBOARD__LOAN_APPLICATION_STATUS}:</Text>
                        </View>

                        {result(accountInfo, 'code', '') === 'LoanKPR' ? // ini diganti jd LoanKPR harusnya
                          ...
                          statusOpeningAccount !== '' ?
                            ...
                            :
                            <Touchable onPress={approveAplication(accountInfo)} style={styles.reloadUpdates}>
                              <Text style={styles.reloadTextLoan}>{language.TAP__SEE_UPDATE}</Text>
                              <View style={styles.reloadIcon}><SimasIcon name='reload' style={styles.reloadIcon} size={15}/></View>
                            </Touchable>
                        }
                      </View>
                  ...
                  {nextStepOpeningAccount === '1' || nextStepOpeningAccount === '2' || nextStepOpeningAccount === '3' ?
                    ...
                    : null }
                  {(nextStepOpeningAccount === '99' && statusOpeningAccount === '30') || (nextStepOpeningAccount === '0' && statusOpeningAccount === '30') || (nextStepOpeningAccount === '99' && statusOpeningAccount === '99') ?
                    <View style={styles.boxedInfo}>
                      <View>
                        <SimasIcon style={styles.iconColor} name='caution-circle' size={15}/>
                      </View>
                        ...
                        <View style={styles.detailContainer}>
                          <View style={styles.row}>
                            ...
                            {itemTheme.displayEye ?
                              <Touchable onPress={setVisibility} style={{width: 30, height: 30}}>
                      ...
                      <ElevatedView elevation={2} style={styles.imageContainer}>
                        <ImageBackground source={itemTheme.image} borderRadius={15} style={styles.backgroundImage}>
                          <View style={styles.detailContainer}>
                            ...
                            {itemTheme.displayArrow && <Touchable  onPress={accountStatus === 'dormant' ? goToshowDormantModal : accountInfo.NamaProgram === 'MudharabahMuqayyadah' ? getMmqDataDetail(accountInfo) : availableBalances === null ? loadBalances : itemType === 'savingAccount' || itemType === 'currentAccount' || itemType === 'rdn' ? this.showDetail() : onItemPress(accountInfo)} style={styles.arrowIconContainer3}>
                              {
