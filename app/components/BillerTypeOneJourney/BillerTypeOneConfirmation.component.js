...
import {formatBillDetails, formatDataDetailList, currencyFormatter, getDayName} from '../../utils/transformer.util';
import map from 'lodash/map';
import {language} from '../../config/language';
import styles from './BillerTypeOneConfirmation.style';
...
export default class BillerTypeOneConfirmation extends Component {
  static propTypes = {
  ...
  render () {
    const {accountNo = {}, resData = {}, dataDetail = [], isUseSimas, isADebit,
      biller = {}, handleSubmit, couponUse, goToCoupon, showFavorite = noop,
      favoriteBill, removeFavorite, billerFavorite, isNewBiller, isAutoDebit, autoDebitDate} = this.props;
    ...
    const showAmount = !(isADebit && resData.amountNumber === 1);
              ...
              <Collapsible collapsed={this.state.summaryCollapsed} refName='summary'>
                ...
                <View style={styles.row}>
                  <Text style={styles.robotoLight}>{language.GENERIC_BILLER__FEE_TXT} </Text>
                  {
                    isUseSimas ? 
                      <View style={styles.rowSimasPoin}>
                        <Text style={styles.robotoLight}>{isNewBiller === true ? currencyFormatter(resData.totalBankCharge) : currencyFormatter(resData.bankCharge)}  </Text>
                        <Image source={Poin} style={styles.poinColapsible}/>
                      </View>
                      : isNewBiller === true ?
                        <View style={styles.row}>
                          <Text style={styles.robotoLight}>{language.GENERIC_BILLER__FEE_TXT} </Text>
                          <Text style={styles.robotoLight}>Rp {currencyFormatter(resData.totalBankCharge)}</Text>
                        </View>
                        : 
                        <View style={styles.row}>
                          <Text style={styles.robotoLight}>{language.GENERIC_BILLER__FEE_TXT} </Text>
